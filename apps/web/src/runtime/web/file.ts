...
export function createWebFileRuntime(
  ...
  function createMarkdownFileRouteUrl(path: string) {
  ...
  async function openMarkdownFileRouteInNewWindow(path: string) {
    const routeUrl = createMarkdownFileRouteUrl(path);
    if (!routeUrl) return;
  ...
  return {
    ...
    async resolveMarkdownFolder(path) {
      const { directory } = await directoryForPath(path);

      return {
        name: directory.name || baseNameFromPath(path),
        path
      };
    },
    async resolveWorkspaceResourceRoot() {
      throw new Error("Workspace resource management requires the desktop runtime.");
    },
