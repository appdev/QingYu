...
use crate::settings::model::{validate_portable_settings_bytes, PORTABLE_SETTINGS_MAX_BYTES};
use crate::settings::service::SettingsPublicationEvent;
use crate::storage::{
...
const SETTINGS_FILE_MAX_BYTES: u64 = PORTABLE_SETTINGS_MAX_BYTES as u64;
...
pub enum PortableSettingsJournalPhase {
    Prepared,
    Reconcile,
    Publication,
}
...
pub struct PortableSettingsJournal {
    pub expected_portable_revision: String,
    pub prepared_manifest_revision: Option<String>,
    pub applied_portable_revision: Option<String>,
    pub phase: PortableSettingsJournalPhase,
    staged_base64: Option<String>,
    pub expected_local_hash: Option<String>,
    pub publication_events: Vec<SettingsPublicationEvent>,
}

impl PortableSettingsJournal {
    pub fn prepared(revision: impl Into<String>, staged: Option<&[u8]>) -> Self {
        Self {
    ...
    pub fn staged_bytes(&self) -> Result<Option<Vec<u8>>, String> {
        let bytes = self.decoded_staged_bytes()?;
        if let Some(bytes) = &bytes {
            if bytes.len() as u64 > SETTINGS_FILE_MAX_BYTES
                || validate_portable_settings_bytes(bytes).is_err()
            {
    ...
    pub fn set_staged_bytes(&mut self, staged: Option<&[u8]>) {
        self.staged_base64 = staged.map(|bytes| STANDARD_NO_PAD.encode(bytes));
    }
}

pub fn capture_portable_settings_manifest_revision(
...
impl SettingsFileState {
    ...
    pub fn matches_hash(&self, expected: Option<&str>) -> bool {
...
pub fn capture_settings_file_state(app_data: &Path) -> Result<SettingsFileState, String> {
...
pub fn replace_portable_settings_stage(
    scope: &RemoteSyncScope,
    bytes: Option<&[u8]>,
) -> Result<(), String> {
...
pub fn read_portable_settings_pending(
    scope: &RemoteSyncScope,
...
pub fn portable_settings_pending_contains_legacy_mcp(
    scope: &RemoteSyncScope,
) -> Result<bool, String> {
    ...
    Ok(validate_portable_settings_bytes(&sanitized).is_ok())
}
...
pub fn write_portable_settings_pending(
    scope: &RemoteSyncScope,
...
pub fn clear_portable_settings_pending(scope: &RemoteSyncScope) -> Result<(), String> {
    replace_control_file(scope, PENDING_FILE_NAME, None)
}

pub fn clear_portable_settings_manifest(scope: &RemoteSyncScope) -> Result<(), String> {
...
mod tests {
    ...
    fn settings_file_state_detects_content_changes_and_deletion() {
        ...
        fs::write(&path, br#"{"appearanceMode":"light"}"#).expect("write first settings");
        let first = capture_settings_file_state(app_data.path()).expect("capture first settings");

        fs::write(&path, br#"{"appearanceMode":"dark"}"#).expect("replace settings");
        let second =
            capture_settings_file_state(app_data.path()).expect("capture replaced settings");
        assert_ne!(first, second);

        fs::remove_file(path).expect("remove settings");
        let missing =
            capture_settings_file_state(app_data.path()).expect("capture missing settings");
    ...
    fn settings_file_state_detects_same_content_file_replacement() {
        let app_data = tempdir().expect("temporary app data");
        let path = app_data.path().join("settings.json");
        fs::write(&path, br#"{"appearanceMode":"light"}"#).expect("write settings");
        let first = capture_settings_file_state(app_data.path()).expect("capture first settings");

        let replacement = app_data.path().join("replacement.json");
        fs::write(&replacement, br#"{"appearanceMode":"light"}"#).expect("write replacement");
        fs::rename(replacement, path).expect("install replacement");
        let second = capture_settings_file_state(app_data.path()).expect("capture replacement");

        assert_ne!(first, second);
    }

    #[test]
    fn settings_file_state_exposes_the_exact_bytes_and_hash() {
        let app_data = tempdir().expect("temporary app data");
        let settings = br#"{"language":"en"}"#;
        fs::write(app_data.path().join("settings.json"), settings).expect("write settings");

        let state = capture_settings_file_state(app_data.path()).expect("capture settings");

        assert_eq!(state.bytes(), Some(settings.as_slice()));
        assert_eq!(
            state.hash(),
            Some("114a6e8f5c43bea09a4a73b24b44b030440a6f3be212bbe943becdb363f15e29")
        );
        assert!(state.matches_hash(state.hash()));
        assert!(!state.matches_hash(Some(
            "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
        )));
    }

    #[test]
    fn journal_restage_replaces_the_recovery_payload() {
        let mut journal =
            PortableSettingsJournal::prepared("portable-revision", Some(br#"{"language":"en"}"#));

        journal.set_staged_bytes(Some(br#"{"language":"zh-CN"}"#));

        assert_eq!(
            journal.staged_bytes().expect("decode staged").as_deref(),
            Some(br#"{"language":"zh-CN"}"#.as_slice())
        );
    }

    #[test]
    fn stage_replacement_and_clear_are_observable_through_scope_source_state() {
        let app_data = tempdir().expect("temporary app data");
        let app_data_root = app_data.path().canonicalize().expect("canonical app data");
        let scope = RemoteSyncScope::portable_settings(
            &app_data_root,
            app_data_root.join("sync-state/settings-stage"),
            "manifest.json",
        )
        .expect("portable settings scope");

        replace_portable_settings_stage(&scope, Some(br#"{"language":"en"}"#))
            .expect("replace settings stage");
        assert_eq!(
            capture_settings_file_state(scope.source_root())
                .expect("capture staged settings")
                .bytes(),
            Some(br#"{"language":"en"}"#.as_slice())
        );

        replace_portable_settings_stage(&scope, None).expect("clear settings stage");
        assert!(capture_settings_file_state(scope.source_root())
            .expect("capture cleared stage")
            .is_missing());
    }

    #[test]
    fn pending_journal_clear_is_idempotent() {
        let app_data = tempdir().expect("temporary app data");
        let app_data_root = app_data.path().canonicalize().expect("canonical app data");
        let scope = RemoteSyncScope::portable_settings(
            &app_data_root,
            app_data_root.join("sync-state/settings-clear-journal"),
            "manifest.json",
        )
        .expect("portable settings scope");
        let journal =
            PortableSettingsJournal::prepared("portable-revision", Some(br#"{"language":"en"}"#));
        write_portable_settings_pending(&scope, &journal).expect("write pending journal");

        clear_portable_settings_pending(&scope).expect("clear pending journal");
        assert!(read_portable_settings_pending(&scope)
            .expect("read cleared journal")
            .is_none());
        clear_portable_settings_pending(&scope).expect("clear absent pending journal");
    }

    #[test]
    fn manifest_revision_tracks_exact_manifest_content() {
        let app_data = tempdir().expect("temporary app data");
        let app_data_root = app_data.path().canonicalize().expect("canonical app data");
        let settings_state = app_data_root.join("sync-state/settings-manifest-revision");
        let scope =
            RemoteSyncScope::portable_settings(&app_data_root, &settings_state, "manifest.json")
                .expect("portable settings scope");
        assert_eq!(
            capture_portable_settings_manifest_revision(&scope).expect("missing revision"),
            None
        );
        fs::write(
        ...
        .expect("write settings manifest");

        assert_eq!(
            capture_portable_settings_manifest_revision(&scope).expect("manifest revision"),
            Some(
                "sha256:b64836f2a140679bbd4cdea2d614808d7558eaf466c872d613a214cab8bcdf9c"
                    .to_string()
            )
        );
    }
    ...
    fn pending_journal_round_trips_the_exact_staged_settings_bytes() {
        ...
        write_portable_settings_pending(&scope, &journal).expect("write journal");
        let restored = read_portable_settings_pending(&scope)
            .expect("read journal")
            .expect("pending journal");

        assert_eq!(
            restored.staged_bytes().expect("decode staged").as_deref(),
    ...
    fn pending_journal_detects_the_legacy_mcp_schema() {
        ...
        assert!(
            portable_settings_pending_contains_legacy_mcp(&scope).expect("inspect legacy journal")
        );
    }
    ...
    fn invalid_non_mcp_pending_settings_are_not_classified_as_legacy_mcp() {
        ...
        assert!(!portable_settings_pending_contains_legacy_mcp(&scope)
            .expect("inspect invalid journal"));
    ...
    fn pending_mcp_publication_event_marks_an_old_schema_journal() {
        ...
        assert!(portable_settings_pending_contains_legacy_mcp(&scope)
            .expect("inspect publication event"));
    }
    ...
    fn clearing_the_portable_manifest_preserves_sibling_note_state() {
        ...
        fs::create_dir_all(&settings_state).expect("settings state");
        fs::create_dir_all(&notes_state).expect("notes state");
        fs::write(notes_state.join("manifest.json"), b"notes-manifest").expect("notes manifest");
        let scope = RemoteSyncScope::portable_settings(
            &app_data_root,
        ...
        clear_portable_settings_manifest(&scope).expect("clear settings manifest");

        assert!(!settings_state.join("engine/manifest.json").exists());
        assert_eq!(
            fs::read(notes_state.join("manifest.json")).expect("read notes manifest"),
            b"notes-manifest"
        );
    }
    ...
    fn settings_file_state_rejects_symlinks() {
        ...
        let app_data = tempdir().expect("temporary app data");
        let outside = tempdir().expect("temporary outside data");
        fs::write(outside.path().join("settings.json"), b"{}").expect("outside settings");
        symlink(
