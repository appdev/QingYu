...
use super::repository::{SyncManifest, SyncManifestEntry, SyncManifestRepository};
use crate::storage::{
...
pub async fn with_remote_sync_execution_lock<Operation, OperationFuture, Output>(
...
pub async fn execute_portable_settings_sync_locked<Backend, Reconcile>(
    settings_scope: &RemoteSyncScope,
...
pub async fn execute_remote_sync_pair_locked<NotesBackend, SettingsBackend, Reconcile>(
    notes_scope: &RemoteSyncScope,
    notes_backend: &NotesBackend,
    settings_scope: &RemoteSyncScope,
    settings_backend: &SettingsBackend,
...
pub fn complete_remote_first_restore_locked(scope: &RemoteSyncScope) -> Result<(), String> {
    if !scope.remote_first_restore() {
        return Ok(());
    }
...
pub fn preserve_remote_settings_conflict(
    scope: &RemoteSyncScope,
    bytes: Option<&[u8]>,
) -> Result<PathBuf, String> {
...
#[doc(hidden)]
pub fn preserve_remote_settings_conflict_with_directory_syncs<SyncChild, SyncParent>(
    scope: &RemoteSyncScope,
    bytes: Option<&[u8]>,
...
mod tests {
    ...
    fn test_block_on<Output>(future: impl Future<Output = Output>) -> Output {
    ...
    fn prepared_notes_scope_reads_ignore_rules_from_the_retained_directory() {
        test_block_on(async {
            ...
            let retained_directory =
                crate::storage::open_canonical_directory_nofollow(&source).unwrap();
    ...
    fn retried_remote_first_restore_keeps_its_incomplete_checkpoint() {
        test_block_on(async {
            ...
            let first_scope = RemoteSyncScope::notes_from_prepared_directory(
                root.clone(),
                crate::storage::open_canonical_directory_nofollow(&root).unwrap(),
                &state,
                "fake-manifest.json",
