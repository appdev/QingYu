...
pub const S3_LOGICAL_EMPTY_HEADER: &str = "x-amz-meta-qingyu-logical-empty";
pub const S3_LOGICAL_EMPTY_SENTINEL: &[u8] = b"\0";
...
impl S3Connection {
    ...
    pub fn new_with_addressing_style(
        endpoint_url: &str,
        region: &str,
        bucket: &str,
        access_key_id: &str,
    ...
    pub const fn addressing_style_name(&self) -> &'static str {
        match self.addressing_style {
            S3AddressingStyle::Auto => "auto",
            S3AddressingStyle::Path => "path",
            S3AddressingStyle::VirtualHosted => "virtual-hosted",
        }
    }
...
#[derive(Clone, Copy)]
pub enum S3Payload<'a> {
    Bytes(&'a [u8]),
    Empty,
    LogicalEmpty,
}

pub fn s3_bucket_url(connection: &S3Connection) -> Result<Url, String> {
...
pub fn s3_object_url(connection: &S3Connection, object_segments: &[String]) -> Result<Url, String> {
