...
impl S3Backend {
    ...
    #[allow(dead_code)] // Used when the Kernel production executor is composed.
...
#[allow(dead_code)] // Used by the production executor's already-validated plan.
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  <NextContinuationToken>next token</NextContinuationToken>
    ...
    fn upload_403_returns_typed_access_denied_without_object_path() {
        ...
        let error = test_block_on(backend.upload("private/面试.md", b"body", None))
    ...
    fn upload_retries_a_transient_response_and_verifies_the_final_object() {
        ...
        let identity = test_block_on(backend.upload("note.md", b"body", None))
    ...
    fn ambiguous_create_retry_requests_a_replan_instead_of_overwriting() {
        ...
        let error = test_block_on(backend.upload("note.md", b"body", None))
    ...
    fn upload_precondition_failure_requests_a_replan_without_retrying() {
        ...
        let error = test_block_on(backend.upload("note.md", b"new", Some("etag:old-etag;len:3")))
    ...
    fn zero_length_upload_encodes_logical_empty_with_metadata() {
        ...
        test_block_on(backend.upload("empty.md", b"", None))
    ...
    fn logical_empty_download_decodes_the_signed_sentinel() {
        ...
        let bytes = test_block_on(backend.download("empty.md", "etag:empty-etag;len:1"))
    ...
    fn list_retries_when_a_success_response_body_is_truncated() {
        ...
        let files = test_block_on(backend.list_files())
    ...
    fn download_retries_when_a_success_response_body_is_truncated() {
        ...
        let bytes = test_block_on(backend.download("note.md", "etag:etag-1;len:4"))
    ...
    fn delete_404_remains_a_success() {
        ...
        test_block_on(backend.delete("note.md", "etag:etag-1;len:4"))
    ...
    fn delete_precondition_failure_requests_a_replan_without_retrying() {
        ...
        let error = test_block_on(backend.delete("note.md", "etag:etag-1;len:4"))
...
<ListBucketResult xmlns="http://s3.amazonaws.com/doc/2006-03-01/">
  ...
  <Contents>
    ...
    fn connection_test_s3_is_signed_get_and_keeps_remote_fixture_unchanged() {
        ...
        let checked_target =
            test_block_on(backend.test_connection()).expect("read-only S3 probe should pass");
        handle.join().expect("fixture should finish");
    ...
    fn connection_test_s3_error_excludes_credentials_signed_query_and_response_body() {
        ...
        let error = test_block_on(backend.test_connection())
    ...
    fn connection_test_s3_rejects_redirects_without_visiting_location() {
        for (status, reason) in [(301, "Moved Permanently"), (307, "Temporary Redirect")] {
            ...
            let result = test_block_on(backend.test_connection());
