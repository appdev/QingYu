//! Remote sync backend boundary.

use std::collections::BTreeMap;

use sha2::{Digest, Sha256};

pub fn sync_state_key(namespace: &str, fields: &[&[u8]]) -> String {
...
impl ValidRemoteRoot {
    pub fn parse(value: &str) -> Result<Self, String> {
        normalize_remote_root(value)
            ...
            .ok_or_else(|| "sync-remote-root-invalid: The remote root is invalid.".to_string())
    }
    ...
    pub fn notes_prefix(&self) -> String {
...
fn normalize_remote_root(value: &str) -> Option<String> {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SyncProviderOperation {
    Catalog,
    Delete,
    Download,
    List,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct RemoteSyncDiagnostic {
    pub category: SyncFailureCategory,
    pub code: String,
    pub http_status: Option<u16>,
    pub method: Option<String>,
    pub object_id: Option<String>,
    pub operation: SyncProviderOperation,
...
impl RemoteSyncError {
    ...
    pub fn diagnostic(diagnostic: RemoteSyncDiagnostic) -> Self {
        let message = format!(
            "{}: S3 {} failed.",
            diagnostic.code,
    ...
    pub fn details(&self) -> Option<&RemoteSyncDiagnostic> {
...
pub trait RemoteSyncBackend: Sync {
...
mod tests {
    ...
    #[test]
    fn unclassified_remote_error_never_treats_unsafe_text_as_a_public_code() {
        let safe = RemoteSyncError::unclassified("manifest-write-failed: unavailable");
        let unsafe_text = RemoteSyncError::unclassified("Access denied for /Users/alice/Notes");

        assert_eq!(safe.safe_code(), "manifest-write-failed");
        assert_eq!(unsafe_text.safe_code(), "sync-run-failed");
