//! Shared workspace control-path policy.

use std::ffi::OsStr;
use std::path::Path;

pub const QINGYU_CONTROL_DIR: &str = ".qingyu";
pub const LEGACY_SYNC_DIR: &str = ".markra-sync";
pub const SYNC_MUTATION_STAGING_PREFIX: &str = ".markra-sync-stage-";
...
pub fn is_qingyu_control_directory_name(name: &OsStr) -> bool {
    name.to_str()
        .is_some_and(is_reserved_control_directory_name)
}

pub fn path_contains_qingyu_control_directory(path: &Path) -> bool {
    path.components()
        .any(|component| is_qingyu_control_directory_name(component.as_os_str()))
}

pub fn is_protected_sync_relative_path(path: &str) -> bool {
