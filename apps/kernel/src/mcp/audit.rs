...
impl AuditSink {
    pub fn new(app_data_dir: &Path, mut policy: AuditPolicy) -> Self {
    ...
    pub fn update_policy(&self, mut policy: AuditPolicy) -> Result<(), AuditError> {
        policy.retention_days = policy.retention_days.clamp(1, 365);
        policy.max_entries = policy.max_entries.clamp(100, 100_000);
    ...
    pub fn preflight(&self) -> Result<(), AuditError> {
        let policy = self
            .policy
            .lock()
            .map_err(|_| AuditError::unavailable())?
    ...
    pub fn list(&self, offset: usize, limit: usize) -> Result<Vec<AuditEntry>, AuditError> {
        let _guard = self
            .operation_lock
            .lock()
