...
#[derive(Clone)]
pub struct VerifiedDocumentHandle {
    workspace: ResolvedWorkspace,
    relative_path: PathBuf,
    kernel_document_id: Option<crate::contract::DocumentId>,
}
...
impl VerifiedDocumentHandle {
    ...
    pub fn workspace(&self) -> &ResolvedWorkspace {
        &self.workspace
    }

    pub fn kernel_document_id(&self) -> Option<&crate::contract::DocumentId> {
        self.kernel_document_id.as_ref()
    }

    pub fn open_file(&self) -> Result<File, HandleError> {
        self.workspace
    ...
    pub fn open_parent_dir(&self) -> Result<Dir, HandleError> {
        self.workspace
            .revalidate_authority()
            .map_err(HandleError::from_workspace)?;
...
#[derive(Clone)]
pub struct VerifiedFolderHandle {
    workspace: ResolvedWorkspace,
    relative_path: PathBuf,
}
...
impl VerifiedFolderHandle {
    ...
    pub fn open_dir(&self) -> Result<Dir, HandleError> {
        self.workspace
            .revalidate_authority()
            .map_err(HandleError::from_workspace)?;
...
impl HandleSigner {
    ...
    pub fn issue_kernel_document(
        &self,
        workspace_id: Uuid,
        workspace_generation: u64,
        relative_path: &str,
        kernel_document_id: &crate::contract::DocumentId,
    ) -> Result<String, HandleError> {
        let path = validate_relative_path(relative_path, false)?;
        if !is_markdown_path(&path) {
            return Err(HandleError::invalid());
    ...
    pub fn verify_document(
        ...
        let kernel_document_id = payload
            .kernel_document_id
            .map(|value| {
                serde_json::from_value::<crate::contract::DocumentId>(
    ...
    pub fn verify_document_in_workspace(
        &self,
        handle: &str,
        workspace_id: Uuid,
        registry: &WorkspaceRegistry,
