...
use super::{
    ...
    kernel_port::{McpKernelFailure, McpKernelPort},
...
pub struct McpServices {
    pub config: Arc<McpConfigManager>,
    pub workspaces: Arc<WorkspaceRegistry>,
    pub handles: Arc<HandleSigner>,
    pub kernel: Arc<dyn McpKernelPort>,
...
pub struct QingYuMcpHandler {
    runtime: Option<Arc<HandlerRuntime>>,
}

impl QingYuMcpHandler {
    pub fn new(services: McpServices, confirmation: Arc<dyn ConfirmationPresenter>) -> Self {
        Self {
    ...
    pub fn list_tools_current(&self) -> Result<Vec<Tool>, McpToolFailure> {
    ...
    pub async fn call_tool_current(
        &self,
        name: &str,
        arguments: Value,
    ...
    pub async fn call_tool_current_with_cancellation(
        &self,
        name: &str,
        arguments: Value,
...
mod tests {
    ...
    fn kernel_revision_conflicts_preserve_the_safe_current_revision() {
        let failure = super::failure_from_kernel(McpKernelFailure::ApiRevisionConflict {
            code: crate::contract::ErrorCode::SyncConfigRevisionConflict,
