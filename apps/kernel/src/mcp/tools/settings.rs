...
use crate::contract::{PatchSettingsRequest, Revision, SettingEntryDto, SettingValueDto};
...
fn snapshot_value(snapshot: crate::contract::SettingsSnapshotDto) -> ToolResult {
    let values = snapshot
        .values
        .into_iter()
        .map(|entry| {
