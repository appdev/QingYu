...
pub struct AuthorizedWorkspaceConfig {
...
impl ResolvedWorkspace {
    pub fn revalidate_authority(&self) -> Result<(), WorkspaceError> {
...
impl WorkspaceRegistry {
    ...
    pub fn for_application_data(app_config_root: &Path, app_data_root: &Path) -> Self {
    ...
    pub fn publish_authority_epoch(&self, epoch: u64) -> Result<u64, WorkspaceError> {
        let _authority = self
            .authority_gate
            .write()
            .map_err(|_| WorkspaceError::state())?;
    ...
    pub fn commit_published_current(
        &self,
        epoch: u64,
        path: Option<&Path>,
    ...
    pub fn clear_current(&self) -> Result<(), WorkspaceError> {
        let _authority = self
            .authority_gate
            .write()
            .map_err(|_| WorkspaceError::state())?;
    ...
    pub fn remove(&self, workspace_id: Uuid) -> Result<(), WorkspaceError> {
        let _authority = self
            .authority_gate
            .write()
            .map_err(|_| WorkspaceError::state())?;
    ...
    pub fn resolve(&self, workspace_id: Uuid) -> Result<ResolvedWorkspace, WorkspaceError> {
        let workspaces = self
            .workspaces
            .read()
            .map_err(|_| WorkspaceError::state())?;
    ...
    pub fn require_primary_workspace(&self) -> Result<ResolvedWorkspace, WorkspaceError> {
        let workspaces = self
            .workspaces
            .read()
            .map_err(|_| WorkspaceError::state())?;
    ...
    pub fn list_safe(&self) -> Vec<SafeWorkspace> {
    ...
    pub fn configs(&self) -> Result<Vec<AuthorizedWorkspaceConfig>, WorkspaceError> {
    ...
    #[cfg(test)]
    pub fn set_authority_read_hook(&self, hook: impl Fn() + Send + Sync + 'static) {
        *self
            .authority_read_hook
            .write()
