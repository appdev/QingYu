...
fn application_about_metadata(app_name: &'static str) -> AboutMetadata<'static> {
    AboutMetadata {
...
fn native_about_dialog_title(metadata: &AboutMetadata<'_>) -> String {
    format!("About {}", metadata.name.as_deref().unwrap_or("QingYu"))
}
...
fn show_native_app_about_for_window<R: tauri::Runtime>(
    ...
    let language = resolve_startup_language(&window.app_handle().config().identifier);
    let labels = crate::menu_labels::for_language(language);
    show_windows_native_about_dialog(hwnd, application_about_metadata(labels.app_name));
    Ok(())
}
...
mod tests {
    ...
    fn menu_labels_follow_the_selected_startup_language() {
        ...
        let traditional_chinese = crate::menu_labels::for_language(AppLanguage::ZhTw);

        assert_ne!(simplified_chinese.file, english.file);
        assert_ne!(simplified_chinese.new_document, english.new_document);
        assert_eq!(english.app_name, "QingYu");
        assert_eq!(simplified_chinese.app_name, "轻语");
        assert_eq!(traditional_chinese.app_name, "輕語");
        assert_eq!(simplified_chinese.hide, "隐藏轻语");
        assert_eq!(simplified_chinese.quit, "退出轻语");
        assert_eq!(traditional_chinese.hide, "隱藏輕語");
        assert_eq!(traditional_chinese.quit, "結束輕語");
    }
    ...
    fn about_metadata_includes_version_and_github_link() {
        let labels = crate::menu_labels::for_language(AppLanguage::En);
        let metadata = application_about_metadata(labels.app_name);

        assert_eq!(metadata.name.as_deref(), Some("QingYu"));
    ...
    fn native_about_dialog_uses_application_metadata() {
        ...
        assert_eq!(native_about_dialog_title(&metadata), "About QingYu");
    ...
    #[test]
    fn routes_settings_window_menu_command_to_the_focused_frontend() {
        assert!(is_frontend_menu_command("openSettings"));
        assert!(!is_frontend_menu_command("unknownSettingsCommand"));
    }
