...
fn standard_install_dirs() -> Vec<PathBuf> {
    ...
    if cfg!(windows) {
        ...
        if let Some(local_app_data) = env::var_os("LOCALAPPDATA") {
            dirs.push(PathBuf::from(local_app_data).join("QingYu").join("bin"));
        }
    } else {
