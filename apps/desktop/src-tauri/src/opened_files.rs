...
fn opened_markdown_paths_from_args_with_optional_cwd(
    ...
    args.into_iter()
        ...
        .filter_map(|arg| {
            ...
            opened_markdown_path_from_path(launch_arg_path(&arg, cwd))
        })
        .collect()
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn preserves_trailing_spaces_in_unix_launch_argument_paths() {
        let root = test_root("trailing-space-args");
        let notebook = root.join("Notes ");
        ...
        assert_eq!(
            ...
            vec![notebook
                .canonicalize()
                .expect("trailing-space folder should canonicalize")
                .to_string_lossy()
                .to_string()]
        );
