...
pub(super) fn markdown_search_line(
    content: &str,
...
pub(super) fn markdown_search_snippet(
    line_text: &str,
    column_number: usize,
    match_length: usize,
) -> String {
...
mod tests {
    ...
    fn uses_root_markraignore_when_searching_workspace_files() {
        ...
        let legacy_sync = root.join(".markra-sync");
        ...
        fs::create_dir_all(&legacy_sync).expect("legacy sync folder should be created");
        fs::write(
        ...
        fs::write(qingyu_sync.join("private.md"), "shared search marker")
            .expect("QingYu private markdown should be created");
        fs::write(legacy_sync.join("manifest.md"), "shared search marker")
            .expect("legacy sync markdown should be created");
    ...
    #[test]
    fn excludes_control_directory_ascii_case_variants_from_search() {
        let root = std::env::temp_dir().join(format!(
            "markra-control-case-search-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after epoch")
                .as_nanos()
        ...
        let qingyu_control = root.join(".QINGYU");
        let legacy_sync = root.join(".MARKRA-SYNC");
        fs::create_dir_all(&qingyu_control).expect("case-variant QingYu folder should be created");
        fs::create_dir_all(&legacy_sync).expect("case-variant legacy folder should be created");
        fs::write(root.join("visible.md"), "shared marker")
            .expect("visible note should be created");
        fs::write(qingyu_control.join("private.md"), "shared marker")
            .expect("private QingYu note should be created");
        fs::write(legacy_sync.join("manifest.md"), "shared marker")
            .expect("private legacy note should be created");

        let search = search_markdown_files_for_path_blocking(
            root.to_string_lossy().to_string(),
            "shared marker".to_string(),
            false,
            None,
            None,
            None,
        ...
        assert_eq!(search.results[0].file.relative_path, "visible.md");
