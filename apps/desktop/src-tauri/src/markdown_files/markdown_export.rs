...
fn export_markdown_file_with_importer_in_registry(
    ...
    folder: String,
    references: Vec<MarkdownExportReference>,
    import: impl FnMut(&Path, &Path, &str) -> Result<String, String>,
) -> Result<PathBuf, String> {
    let _mutation = registry.acquire_mutation(&[PathBuf::from(&parent_path)])?;
...
mod tests {
    ...
    fn rejects_an_export_parent_blocked_by_sync_before_creating_the_bundle() {
        ...
        let root_path = root.path().canonicalize().expect("temporary root should resolve");
        ...
        let result = export_markdown_file_with_importer_in_registry(
            ...
            "assets".to_string(),
            Vec::new(),
            |_, _, _| panic!("resource importer must not run for a guarded export"),
        );
