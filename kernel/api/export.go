...
func exportPreview(c *gin.Context) {
	...
	if userAgentStr != "" {
		...
		name, _ := ua.Browser()
		// Chrome、Edge、轻语桌面端及兼容的思源桌面端不需要替换 CSS 变量
		if !ua.Mobile() && (name == "Chrome" || name == "Edge" || strings.Contains(userAgentStr, "Electron") || util.IsNativeAppUserAgent(userAgentStr)) {
			fillCSSVar = false
		}
	}
