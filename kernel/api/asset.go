...
func removeUnusedAsset(c *gin.Context) {
	ret := gulu.Ret.NewResult()
	defer c.JSON(http.StatusOK, ret)
	if err := awaitMarkdownSaveBarrier(5 * time.Second); err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		return
	}
	...
	p := arg["path"].(string)
	asset, err := model.RemoveUnusedAssetWithError(p)
	if err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		return
...
func removeUnusedAssets(c *gin.Context) {
	...
	paths, err := model.RemoveUnusedAssetsWithError()
	if err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		return
...
func getUnusedAssets(c *gin.Context) {
	...
	if err := awaitMarkdownSaveBarrier(5 * time.Second); err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		ret.Data = []*model.UnusedItem{}
		return
	}

	unusedAssets, err := model.UnusedAssetsWithError(true)
	if err != nil {
		ret.Code = -1
		ret.Msg = err.Error()
		ret.Data = []*model.UnusedItem{}
