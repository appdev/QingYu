...
func getConf(c *gin.Context) {
	...
	// 原生客户端（桌面 Electron、移动端）使用轻语或兼容的思源 UA，照常返回真实路径
...
func setMCP(c *gin.Context) {
	...
	enabled := model.Conf.Api.MCP.Enabled
	readOnly := model.Conf.Api.MCP.ReadOnly
	changed := false
	if enabledValue, exists := arg["enabled"]; exists {
		var ok bool
		enabled, ok = enabledValue.(bool)
		if !ok {
			ret.Code = -1
			ret.Msg = "enabled must be boolean"
			return
		}
		changed = true
	}
	if readOnlyValue, exists := arg["readOnly"]; exists {
		var ok bool
		readOnly, ok = readOnlyValue.(bool)
		if !ok {
			ret.Code = -1
			ret.Msg = "readOnly must be boolean"
			return
		}
		changed = true
	}
	if !changed {
		ret.Code = -1
		ret.Msg = "enabled or readOnly is required"
		return
	}
	model.Conf.Api.MCP.Enabled = enabled
	model.Conf.Api.MCP.ReadOnly = readOnly
	model.Conf.Save()
	ret.Data = model.Conf.Api.MCP
}
