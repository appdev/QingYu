...
const (
	ProviderS3     = 0 // ProviderS3 为 S3 协议对象存储提供的云端存储服务
	ProviderWebDAV = 1 // ProviderWebDAV 为 WebDAV 协议提供的云端存储服务
)

func IsValidSyncProvider(provider int) bool {
	return provider == ProviderS3 || provider == ProviderWebDAV
}

// NormalizeProvider 将不再支持的同步服务重置为 S3，并关闭同步以等待重新配置。
func (sync *Sync) NormalizeProvider() {
	if !IsValidSyncProvider(sync.Provider) {
		sync.Provider = ProviderS3
		sync.Enabled = false
	}
}
