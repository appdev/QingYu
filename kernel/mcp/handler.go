...
func handleToolsList(id any) *JsonRpcResponse {
	toolList := allowedTools(mcpReadOnly())
	return &JsonRpcResponse{
		JsonRpc: "2.0",
...
func handleToolsCall(req *JsonRpcRequest) any {
	...
	if !isToolCallAllowed(t, toolArgs, mcpReadOnly()) {
		return &JsonRpcResponse{
			JsonRpc: "2.0",
			Result: tools.CallToolResult{
				Content: []tools.ContentItem{{Type: "text", Text: "tool action is not allowed in MCP read-only mode"}},
				IsError: true,
			},
			ID: req.ID,
		}
...
func mcpReadOnly() bool {
	return model.Conf != nil && model.Conf.Api != nil && model.Conf.Api.MCP != nil && model.Conf.Api.MCP.ReadOnly
}
