...
func Serve(ginServer *gin.Engine) {
	...
	ginServer.POST("/mcp", model.CheckAuth, model.CheckAdminRole, model.CheckReadonly, checkEnabled, handlePost)
	ginServer.GET("/mcp", model.CheckAuth, model.CheckAdminRole, checkEnabled, func(c *gin.Context) {
		c.Status(http.StatusMethodNotAllowed)
	})
	ginServer.DELETE("/mcp", model.CheckAuth, model.CheckAdminRole, model.CheckReadonly, checkEnabled, handleDelete)
}

func checkEnabled(c *gin.Context) {
	if model.Conf == nil || model.Conf.Api == nil || model.Conf.Api.MCP == nil || !model.Conf.Api.MCP.Enabled {
		c.JSON(http.StatusForbidden, gin.H{"error": "MCP server is disabled"})
		c.Abort()
		return
	}
	c.Next()
