...
var SearchTool = &Tool{
	Name:        "search",
	Description: "Search. Actions: fulltext(query, page=1, pageSize=20, notebook?, path?, type?, subtype?, method?, orderBy?, groupBy?), asset(query, page=1, pageSize=32, ext?, method?, orderBy?) — full-text search inside asset file contents (PDF/Word/Excel/txt etc.), returns matched snippets with <mark> tags; getasset(path) — get the full indexed content of one asset file by its path (e.g. 'assets/foo.pdf').",
	InputSchema: ToolSchema{
		Type: "object",
		Properties: map[string]Property{
			"action":   {Type: "string", Description: "Operation: fulltext, asset, or getasset", Enum: []string{"fulltext", "asset", "getasset"}},
			"query":    {Type: "string", Description: "Search keywords (required for fulltext/asset)"},
			"page":     {Type: "number", Description: "Page number (default 1)"},
			"pageSize": {Type: "number", Description: "Results per page (default 20 for fulltext, 32 for asset)"},
			"notebook": {Type: "string", Description: "Comma-separated notebook IDs to filter (optional, fulltext only)"},
			"path":     {Type: "string", Description: "Comma-separated path prefixes to filter (optional, fulltext only); for getasset, a single asset file path like 'assets/foo.pdf'"},
...
func searchHandler(args map[string]any) (CallToolResult, error) {
	...
	return CallToolResult{
		Content: []ContentItem{{Type: "text", Text: "unknown action '" + action + "', expected one of: [fulltext, asset, getasset]"}},
		IsError: true,
	}, nil
}
