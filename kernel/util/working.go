...
func Boot() {
	...
	workspacePath := flag.String("workspace", "", "dir path of the workspace, default to ~/QingYu/")
	wdPath := flag.String("wd", WorkingDir, "working directory of QingYu")
...
func BootWithFlags(workspacePath, wdPath, port, readOnly, accessAuthCode, lang, mode string, ssl, attachUI, safeMode bool) {
	...
	bootBanner := figure.NewColorFigure("QingYu", "isometric3", "green", true)
	logging.LogInfo("\n" + bootBanner.String())
	logBootInfo()
}
...
var (
	...
	ShortcutsPath      string        // 用户家目录下的快捷方式目录路径 home/.config/qingyu/shortcuts/
...
func initWorkspaceDir(workspaceArg string) {
	...
	if gulu.OS.IsWindows() {
		...
		if userProfile := os.Getenv("USERPROFILE"); "" != userProfile {
			defaultWorkspaceDir = filepath.Join(userProfile, "QingYu")
		}
		...
		defaultWorkspaceDir = filepath.Join(HomeDir, "Library", "Application Support", "QingYu")
	}
...
func UserHomeConfDir() string {
	return filepath.Join(HomeDir, ".config", "qingyu")
}
