...
func generateCACert(certPath, keyPath string) error {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"QingYu"},
			CommonName:   "QingYu Local CA",
		},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
...
func generateServerCert(certPath, keyPath string, caCert *x509.Certificate, caKey any) error {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{
			Organization: []string{"QingYu"},
			CommonName:   "QingYu Local Server",
		},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
...
// GetServerAddrs() returns URLs like "http://192.168.1.1:9806", this function extracts just the IP part
