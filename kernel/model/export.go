...
func processPDFLinkEmbedAssets(pdfCtx *model.Context, assetDests []string, boxID string, removeAssets bool) {
	...
	for _, link := range assetLinks {
		...
		link.URI = strings.ReplaceAll(link.URI, "http://"+util.LocalHost+":"+util.FixedPort+"/export/temp/", "")
		...
		link.URI = strings.ReplaceAll(link.URI, "http://"+util.LocalHost+":"+util.FixedPort+"/", "")
		link.URI, _ = url.PathUnescape(link.URI)
		sourceURI := link.URI
		...
		fileSpecDict, newErr := pdfCtx.XRefTable.NewFileSpecDict(fn, fn, "attached by QingYu", *ir)
		if nil != newErr {
		...
		if 4 == Conf.Export.BlockRefMode { // 脚注+锚点哈希
			...
			ast.Walk(tree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
				...
				if treenode.IsBlockLink(n) {
					defID, _ = trimAppBlockURIPrefix(n.TextMarkAHref)
...
func exportTree(tree *parse.Tree, wysiwyg, keepFold, avHiddenCol bool,
	...
	ast.Walk(ret.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		switch blockRefMode {
		case 2: // 锚文本块链
			blockRefLink := &ast.Node{Type: ast.NodeTextMark, TextMarkTextContent: linkText, TextMarkAHref: appBlockURI(defID)}
			blockRefLink.KramdownIAL = n.KramdownIAL
...
func blockLink2Ref(currentTree *parse.Tree) {
	ast.Walk(currentTree.Root, func(n *ast.Node, entering bool) ast.WalkStatus {
		...
		if treenode.IsBlockLink(n) {
			...
			n.TextMarkBlockRefID, _ = trimAppBlockURIPrefix(n.TextMarkAHref)
