...
func SearchDocs(keyword string, excludeIDs []string) (ret []map[string]string) {
	...
	for _, file := range files {
		...
		if file.isdir {
			...
			if ial := box.docIAL(parentDocPath); nil != ial {
				...
				if err == nil {
					for _, subFile := range subFiles {
						...
						if strings.HasSuffix(subFile.Name(), ".sy") || isMarkdownFileName(subFile.Name()) {
							doc.SubFileCount++
						}
					}
				}
			...
			if isMarkdownFileName(file.name) {
				docs = append(docs, markdownFileDisplayInfo(box.ID, file))
				continue
			}
	...
	case util.SortModeCustom:
		...
		sort.Slice(fileTreeFiles, func(i, j int) bool {
			if fileTreeFiles[i].Sort == fileTreeFiles[j].Sort {
				if "markdown" == fileTreeFiles[i].DocType || "markdown" == fileTreeFiles[j].DocType {
					return util.NaturalCompare(fileTreeFiles[i].Name, fileTreeFiles[j].Name)
				}
...
func moveSorts(rootID, fromBox, toBox string) {
	...
	if nil != bt {
		...
		docs, _, listErr := ListDocTree(toBox, parentPath, util.SortModeUnassigned, false, 102400)
		if listErr != nil {
...
func (box *Box) addMaxSort(parentPath, id string) {
	docs, _, err := ListDocTree(box.ID, parentPath, util.SortModeUnassigned, false, 102400)
	if err != nil {
...
func (box *Box) addMinSort(parentPath, id string) {
	docs, _, err := ListDocTree(box.ID, parentPath, util.SortModeUnassigned, false, 1)
	if err != nil {
		logging.LogErrorf("list doc tree failed: %s", err)
		return
	}
...
func (box *Box) addSort(previousPath, id string) {
	...
	docs, _, err := ListDocTree(box.ID, parentPath, util.SortModeUnassigned, false, Conf.FileTree.MaxListCount)
	if err != nil {
		logging.LogErrorf("list doc tree failed: %s", err)
		return
	}
