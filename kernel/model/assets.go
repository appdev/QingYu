...
func RemoveUnusedAssets() (ret []string) {
	ret, err := RemoveUnusedAssetsWithError()
	if err != nil {
		logging.LogErrorf("remove unused assets failed: %s", err)
	}
	return ret
}

func RemoveUnusedAssetsWithError() (ret []string, err error) {
	ret = []string{}
	var size int64
	unusedAssets, err := UnusedAssetsWithError(false)
	if err != nil {
		return ret, err
	}
...
func RemoveUnusedAsset(p string) (ret string) {
	ret, err := RemoveUnusedAssetWithError(p)
	if err != nil {
		logging.LogErrorf("remove unused asset failed: %s", err)
	}
	return ret
}

func RemoveUnusedAssetWithError(p string) (ret string, err error) {
	p = filepath.ToSlash(strings.TrimPrefix(strings.TrimSpace(p), "/"))
	unusedAssets, err := UnusedAssetsWithError(false)
	if err != nil {
		return "", err
	}
	if !containsUnusedAsset(unusedAssets, p) {
		return "", nil
	}
	absPath := filepath.Join(util.DataDir, p)
	if !filelock.IsExist(absPath) {
...
func containsUnusedAsset(items []*UnusedItem, p string) bool {
	for _, item := range items {
		if item != nil && item.Item == p {
			return true
		}
	}
	return false
...
func UnusedAssets(sorted bool) (ret []*UnusedItem) {
	ret, err := UnusedAssetsWithError(sorted)
	if err != nil {
		logging.LogErrorf("scan unused assets failed: %s", err)
		return []*UnusedItem{}
	}
	return ret
}

func UnusedAssetsWithError(sorted bool) (ret []*UnusedItem, err error) {
	...
	for _, notebook := range notebooks {
		...
		markdownDests, markdownErr := collectMarkdownAssetLinkDests(filepath.Join(util.DataDir, notebook.ID))
		if markdownErr != nil {
			return ret, fmt.Errorf("scan Markdown asset references in notebook [%s] failed: %w", notebook.ID, markdownErr)
		}
		for dest := range markdownDests {
			dests[dest] = true
		}

		removeReferencedAssetPaths(assetsPathMap, dests, linkDestMap)
	}
...
func removeReferencedAssetPaths(assetsPathMap map[string]string, dests, linkDestMap map[string]bool) {
	var folderPaths, filePaths []string
	for dest := range dests {
		...
		if strings.HasSuffix(dest, "/") {
			folderPaths = append(folderPaths, dest)
		} else {
	...
	for asset := range assetsPathMap {
		for _, folderPath := range folderPaths {
			if strings.HasPrefix(asset, folderPath) {
				delete(assetsPathMap, asset)
				break
			}
		}
		for _, filePath := range filePaths {
			if strings.HasPrefix(filePath, asset) {
				delete(assetsPathMap, asset)
				break
			}
		}
...
// allAssetAbsPaths 返回 asset 相对路径（assets/xxx）到绝对路径（F:\QingYu\data\assets\xxx）的映射。
