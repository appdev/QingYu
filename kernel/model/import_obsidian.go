...
func validateObsidianVaultRoot(localPath string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: resolve Vault root: %v", errObsidianVaultUnreadable, err)
	}
	abs = resolved
	...
	if sameObsidianPath(abs, workspace) || gulu.File.IsSubPath(workspace, abs) || gulu.File.IsSubPath(abs, workspace) {
		return "", fmt.Errorf("%w: Vault root and QingYu workspace contain each other", errObsidianVaultUnsafePath)
	}
...
func isObsidianResolvedLink(p string) bool {
	...
	resolvedParent, err := filepath.EvalSymlinks(filepath.Dir(p))
	if err != nil {
		return false
	}
	expected := filepath.Join(resolvedParent, filepath.Base(p))
	return !sameObsidianPath(filepath.Clean(expected), filepath.Clean(resolved))
}
