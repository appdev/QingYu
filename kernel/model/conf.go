...
func InitConf() {
	...
	confExists := gulu.File.IsExist(confPath)
	if confExists {
	...
	if "" != util.Lang {
		...
		if detectErr != nil {
			...
			if len(bcp47Tags) > 0 {
				...
				// 系统语言与轻语支持列表不存在有效匹配时 confidence 为 No，保持默认 en
...
func initMCPConfig(confExists bool) {
	if nil == Conf.Api.MCP {
		// 历史工作空间已经默认暴露 MCP，升级时保持兼容；新工作空间默认关闭。
		Conf.Api.MCP = conf.NewMCP(confExists, !confExists)
	}
}
