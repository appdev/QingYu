...
3. **Frontend verification:** After changes, run `cd app && pnpm run lint` to check code style; when runtime UI verification is needed, agents may run `pnpm dev` and launch the relevant app
4. **Frontend build:** Do NOT run `pnpm build`; use `pnpm dev` for runtime verification to avoid conflicting production bundles
