...
| Filesystem separation | `settings.json` and `sync-config.json` exist only below `/data/config`; operational files remain below `/data/state`. | **Passed on `856c3c10`** through Kernel root-placement, the Docker contract, 11 Web-distribution tests, and 28 contract mutation cases. | Pending final-SHA volume inspection. |
