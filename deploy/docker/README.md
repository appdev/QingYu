...
- `verify-runtime.sh --status` exits with status 78 until the real server path exists.
...
- `QINGYU_INIT_TOKEN` uses value-free Compose environment pass-through. It has no image build argument, image environment value, Compose literal, or Compose default.
- The future server must require the token only when initializing an empty `/data`, consume it once, and never persist or log its plaintext. A subsequent start with initialized data must work with the variable absent.
...
These cases become mandatory when P2 delivers the server entrypoint. Until then they are phase-gated, not skipped as passing tests.

| Case | Setup | Required result |
| --- | --- | --- |
| First initialization | Empty `qingyu-data`; non-empty `QINGYU_INIT_TOKEN` | Server initializes `/data`, becomes ready, and does not expose the token in image metadata, logs, or files. |
...
| One-time token replay | Initialized volume; provide the old token again | Server does not reinitialize, rotate ownership, or overwrite existing data. The final P2 policy must explicitly reject or safely ignore the replay. |
...
2. First-run `QINGYU_INIT_TOKEN` validation and one-time consumption semantics, including fail-closed handling for an empty volume without a token.
