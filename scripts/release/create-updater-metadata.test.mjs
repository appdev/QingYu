...
test("create-updater-metadata prefers the Windows setup updater bundle", () => {
  ...
  const [, msiSignature] = writeBundle(rootDir, "QingYu_0.0.8_windows_x64_en-US.msi");
  const [setupBundle, setupSignature] = writeBundle(rootDir, "QingYu_0.0.8_windows_x64_setup.exe");
...
test("create-updater-metadata selects AppImage bundles for Linux updater metadata", () => {
  const rootDir = makeTempDir();
  const [bundlePath, signaturePath] = writeBundle(rootDir, "QingYu_0.0.8_linux_x64.AppImage");
  const outputPath = path.join(rootDir, "release-metadata.json");
...
test("create-updater-metadata selects renamed macOS updater tarballs", () => {
  const rootDir = makeTempDir();
  const [bundlePath, signaturePath] = writeBundle(rootDir, "QingYu_0.0.8_macos_arm64_updater.app.tar.gz");
  const outputPath = path.join(rootDir, "release-metadata.json");
