...
test("prepare-arch-package creates a reproducible x86_64 PKGBUILD from the release deb", () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "qingyu-arch-package-"));
  const debPath = path.join(rootDir, "release-assets", "Linux-x64", "QingYu_2.4.0_linux_x64.deb");
  ...
  assert.match(pkgbuild, /^pkgname=qingyu$/m);
  ...
  assert.match(pkgbuild, /^noextract=\('QingYu_2\.4\.0_linux_x64\.deb'\)$/m);
...
test("prepare-arch-package fails when the normalized x64 deb is missing", () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "qingyu-arch-package-missing-"));
  fs.mkdirSync(path.join(rootDir, "release-assets"), { recursive: true });

  const result = runPrepareScript(rootDir);

  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /QingYu_2\.4\.0_linux_x64\.deb/);
});

test("prepare-arch-package converts semver prerelease hyphens for PKGBUILD", () => {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "qingyu-arch-package-prerelease-"));
  const debPath = path.join(rootDir, "release-assets", "QingYu_2.5.0-beta.1_linux_x64.deb");
  ...
  assert.match(pkgbuild, /^source=\('QingYu_2\.5\.0-beta\.1_linux_x64\.deb'\)$/m);
});
