...
test("resolve-tauri-build-args disables signing when release secrets are incomplete", () => {
  const result = runResolveArgs({
    ASSET_PLATFORM: "linux",
    ENABLE_SIGNED_RELEASE: "false",
    RELEASE_TAG: "v1.7.5",
    TAURI_BUILD_ARGS: "--target x86_64-unknown-linux-gnu",
  });

  assert.equal(result.status, 0, result.stderr);
  assert.equal(result.stdout.trim(), "--target x86_64-unknown-linux-gnu --no-sign");
});

test("resolve-tauri-build-args enables updater config when release secrets are complete", () => {
  const result = runResolveArgs({
    ASSET_PLATFORM: "macos",
    ENABLE_SIGNED_RELEASE: "true",
    RELEASE_TAG: "v1.7.5",
    TAURI_BUILD_ARGS: "--target aarch64-apple-darwin",
  });
  ...
  assert.equal(
    result.stdout.trim(),
    "--target aarch64-apple-darwin --config tauri.updater.conf.json",
  );
});
