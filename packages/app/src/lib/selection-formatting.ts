...
function selectionHasTaskListItem(state: EditorState) {
  return [state.selection.$from, state.selection.$to].every((position) => {
    for (let depth = position.depth; depth > 0; depth -= 1) {
      const node = position.node(depth);
...
export function readSelectionFormattingActionsFromView(view: EditorView): SelectionFormattingAction[] {
  ...
  if (selectionHasTaskListItem(state)) {
    activeActions.push("taskList");
  } else if (selectionHasAncestor(state, "bullet_list")) {
    activeActions.push("bulletList");
  }
