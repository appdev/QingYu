...
type ManagedPathKind = "posix" | "relative" | "windows-drive" | "windows-unc";

type NormalizedManagedPath = {
  kind: ManagedPathKind;
  value: string;
};

function normalizeManagedPath(path: string): NormalizedManagedPath {
  ...
  normalized = unc
    ? `//${normalized.slice(2).replace(/\/+/gu, "/")}`
    : normalized.replace(/\/+/gu, "/");
  if (normalized !== "/" && !/^[a-z]:\/$/iu.test(normalized)) {
    normalized = normalized.replace(/\/$/u, "");
  }

  const kind: ManagedPathKind = /^[a-z]:\//iu.test(normalized)
    ? "windows-drive"
    : /^\/\/[^/]+\/[^/]+(?:\/|$)/u.test(normalized)
      ? "windows-unc"
...
function managedRelativePathIsSafe(path: string) {
  if (!path || path.startsWith("/") || /^[a-z]:\//iu.test(path)) return false;
...
function managedPathComparisonValue(path: NormalizedManagedPath) {
  return path.kind === "windows-drive" || path.kind === "windows-unc"
    ? path.value.toLowerCase()
    : path.value;
}

function managedPathPrefix(path: NormalizedManagedPath) {
  return path.value === "/" || path.value.endsWith("/") ? path.value : `${path.value}/`;
}

function managedPathIsBelowRoot(rootPath: NormalizedManagedPath, filePath: NormalizedManagedPath) {
  if (
    !rootPath.value ||
    !filePath.value ||
    rootPath.kind === "relative" ||
    filePath.kind !== rootPath.kind
  ) return false;

  const rootComparison = managedPathComparisonValue(rootPath);
  const fileComparison = managedPathComparisonValue(filePath);
  if (fileComparison === rootComparison) return false;
...
export function managedDocumentRelativePath(rootPath: string, filePath: string): string | null {
  ...
  const normalizedFile = normalizeManagedPath(filePath);
  if (!managedPathIsBelowRoot(normalizedRoot, normalizedFile)) return null;

  const rootPrefix = managedPathPrefix(normalizedRoot);
  const relativePath = normalizedFile.value.slice(rootPrefix.length);
  return managedRelativePathIsSafe(relativePath) ? relativePath : null;
}

export function managedDocumentAbsolutePath(rootPath: string, relativePath: string): string | null {
  const normalizedRoot = normalizeManagedPath(rootPath);
  const normalizedRelative = normalizeManagedPath(relativePath);
  if (
    !normalizedRoot.value ||
    normalizedRoot.kind === "relative" ||
    normalizedRelative.kind !== "relative" ||
    !managedRelativePathIsSafe(normalizedRelative.value)
  ) return null;

  const absolutePath = normalizeManagedPath(`${managedPathPrefix(normalizedRoot)}${normalizedRelative.value}`);
  return managedPathIsBelowRoot(normalizedRoot, absolutePath) ? absolutePath.value : null;
}
