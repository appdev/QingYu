...
export type PortableExportSettings = Omit<ExportSettings, "pandocPath">;
...
export function normalizePortableExportSettings(value: unknown): PortableExportSettings {
  const settings = typeof value === "object" && value !== null
    ? value as Partial<PortableExportSettings>
    : {};
...
export function normalizeExportSettings(value: unknown): ExportSettings {
  const settings = typeof value === "object" && value !== null
    ? value as Partial<ExportSettings>
    : {};

  return {
    ...normalizePortableExportSettings(settings),
