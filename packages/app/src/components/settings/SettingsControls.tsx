...
export function SettingsSelect({
  ...
  return (
    <div className={`relative inline-flex items-center ${compact ? "w-full" : ""}`}>
      <select
        className={`h-8 min-w-36 appearance-none rounded-md border border-(--border-default) bg-(--bg-primary) py-0 pr-8 pl-3 text-[12px] leading-5 font-[560] text-(--text-heading) transition-colors duration-150 ease-out hover:bg-(--bg-hover) active:bg-(--bg-hover) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--accent) disabled:cursor-not-allowed disabled:opacity-55 ${compact ? "min-h-11 min-w-11 w-full" : ""}`}
        aria-label={label}
        disabled={disabled}
        value={value}
