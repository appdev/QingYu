...
const retainedTitlebarActionIds = new Set(titlebarActionOptions.map((option) => option.id));
...
function SettingsTitlebarActionsControl({
  ...
  const actions = useMemo(
    () => preferences.titlebarActions.filter((action) => retainedTitlebarActionIds.has(action.id)),
    [preferences.titlebarActions]
  );
  ...
  const updateActions = (titlebarActions: TitlebarActionPreference[]) => {
    onUpdatePreferences({
      ...preferences,
      titlebarActions: [...titlebarActions, ...deferredActions]
    });
  };
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    updateActions(
      reorderTitlebarActions(actions, draggedId, targetId)
        .filter((action) => retainedTitlebarActionIds.has(action.id))
    );
  };
