...
describe("MarkdownFileTreeDrawer", () => {
  ...
  it("keeps settings and sync fixed together in the lower-left", () => {
    ...
    const syncNow = screen.getByRole("button", { name: "Sync now · Succeeded" });
    ...
    expect(footerActions).toContainElement(settings);
    expect(footerActions).toContainElement(syncNow);
    expect(settings).toContainElement(container.querySelector(".lucide-settings"));
    expect(syncNow).toHaveClass("opacity-40");
    expect(syncNow).not.toHaveClass("opacity-70");
  ...
  it("places the Windows open folder action in the sidebar header before search", () => {
    ...
    ).toEqual(["Switch Notebook Directory", "Search Markdown files"]);
    expect(screen.getByRole("button", { name: "Switch Notebook Directory" })).toContainElement(
  ...
  it("keeps settings and sync together while leaving update outside the primary Windows footer actions", () => {
    ...
    expect(primaryActions).not.toContainElement(updateButton);
    expect(footer).toContainElement(updateButton);
    expect(screen.queryByRole("button", { name: "Toggle file list" })).not.toBeInTheDocument();
  ...
  it("omits the sync control when no sync callback is provided", () => {
    render(
      <MarkdownFileTreeDrawer
        currentPath="/vault/Untitled.md"
        files={markdownFiles}
  ...
  it("renders QingYu highlight and inline math in outline titles", () => {
    render(
      <MarkdownFileTreeDrawer
        currentPath="/vault/Untitled.md"
        files={markdownFiles}
