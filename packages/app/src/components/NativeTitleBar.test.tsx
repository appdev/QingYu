...
describe("NativeTitleBar", () => {
  it("renders centered title and file actions inside the drag region", () => {
    ...
    expect(container.querySelector("[data-titlebar-action='save']")).toHaveClass("transition-transform");
  });
  ...
  it("keeps macOS titlebar tabs clear of the markdown files sidebar", () => {
    const { container } = render(
      <NativeTitleBar
        dirty={false}
        documentName="Draft.md"
  ...
  it("opens self-drawn Windows app menu dropdowns from the top chrome", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "QingYu" }));

    expect(screen.getByRole("menu", { name: "QingYu" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("menuitem", { name: "About QingYu" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "QingYu" }));
    fireEvent.click(screen.getByRole("menuitem", { name: "Quit QingYu" }));
  ...
  it("reorders right-side file actions by holding and dragging", async () => {
    ...
    const sourceButton = screen.getByRole("button", { name: "Editor view mode: Preview" });
    mockTitlebarActionRects(["sourceMode", "save", "theme", "viewMode", "history"]);

    fireEvent.mouseDown(sourceButton, { button: 0, clientX: 10, clientY: 10 });
  ...
  it("reorders right-side file actions from right to left by holding and dragging", async () => {
    ...
    fireEvent.mouseDown(saveButton, { button: 0, clientX: 40, clientY: 10 });
  ...
  it("keeps the unified Markdown or folder picker on macOS", () => {
    ...
    const menu = screen.getByRole("menu", { name: "Open Markdown or Folder" });
    fireEvent.click(within(menu).getByRole("menuitem", { name: "Open Markdown File" }));
    expect(openMarkdown).toHaveBeenCalledTimes(1);

    fireEvent.click(screen.getByRole("button", { name: "Open Markdown or Folder" }));
    fireEvent.click(screen.getByRole("menuitem", { name: "Switch Notebook Directory..." }));
