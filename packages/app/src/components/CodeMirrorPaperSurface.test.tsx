...
describe("CodeMirrorPaperSurface", () => {
  ...
  it("adapts QingYu workspace files into plugin-driven document links", () => {
    const onEditorReady = vi.fn();
    render(
      <CodeMirrorPaperSurface
        autoFocus={false}
  ...
  it("enables QingYu extended Markdown previews without rewriting source", () => {
    const onEditorReady = vi.fn();
    const doc = [
      "---",
      "title: Synthetic",
  ...
  it("does not publish transient text selections during IME composition", async () => {
    const onEditorReady = vi.fn();
    const onTextSelectionChange = vi.fn();
    render(
      <CodeMirrorPaperSurface
  ...
  it("keeps text selection context out of editable fenced code", () => {
    const onEditorReady = vi.fn();
    const onTextSelectionChange = vi.fn();
  ...
  it("hosts held selections on source positions without changing Markdown", () => {
    const onEditorReady = vi.fn();
    ...
    act(() => {
      showCodeMirrorSelectionHold(view, {
        from,
        text: "Original",
    ...
    expect(container.querySelector(".markra-selection-hold")?.textContent).toContain("Original");
    expect(view.state.doc.toString()).toBe(doc);
  });

  it("routes pasted image files through QingYu's existing save callback", async () => {
    const onEditorReady = vi.fn();
  ...
  it("routes local attachments and external URLs through QingYu host callbacks", () => {
    const onEditorReady = vi.fn();
