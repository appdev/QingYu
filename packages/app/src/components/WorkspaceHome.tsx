...
export type WorkspaceHomeProps = {
  ...
  shortcuts?: Partial<Record<WorkspaceHomeShortcut, readonly string[]>>;
};
...
function applyWorkspaceHomeContrast(home: HTMLElement) {
  ...
  const brandBase = fitContrast(background, preferred, 1.35);
  const brandSlice = fitContrast(background, preferred, 1.6);
...
function BrandMark({ compact }: { compact: boolean }) {
  const maskSize = compact ? "4.875rem 4.875rem" : "8.25rem 8.25rem";
  ...
  return (
    <div
      className={`relative ${compact ? "h-[4.875rem] w-[2.875rem]" : "h-[8.25rem] w-[4.875rem]"}`}
...
export function WorkspaceHome({
  ...
  const homeStyle = {
    "--workspace-home-brand-base": "color-mix(in oklab, var(--bg-primary) 85%, var(--text-heading))",
    "--workspace-home-brand-slice": "color-mix(in oklab, var(--bg-primary) 80%, var(--text-heading))",
    ...
    >
      <div
        className={`mx-auto grid min-w-0 w-full ${
          compact
            ? "my-auto max-w-[16.25rem] gap-2.5"
            : "my-auto max-w-[16.25rem] gap-4"
        }`}
      >
        ...
        <div className="grid min-w-0 gap-0">
          {availableGroups.map((group, groupIndex) => (
            <div className="contents" key={group[0].key}>
              {groupIndex > 0 ? (
                <div
                  className="mx-1.5 my-[3px] h-px bg-(--border-default)"
                  role="separator"
                />
              ) : null}
              <div className="grid min-w-0 gap-0">
                {group.map(({ callback, key, labelKey, shortcutKey }) => {
                  ...
                  return (
                    <button
                      className={`group relative grid min-w-0 w-full cursor-pointer grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border-0 bg-transparent text-left text-[13px] leading-5 font-normal text-(--workspace-home-text) outline-none transition-[transform,background-color] duration-150 ease-out before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:scale-y-50 before:rounded-full before:bg-(--workspace-home-focus) before:opacity-0 before:transition-[opacity,transform] before:duration-150 hover:bg-(--bg-hover) hover:before:scale-y-100 hover:before:opacity-100 focus-visible:ring-2 focus-visible:ring-(--workspace-home-focus) focus-visible:before:scale-y-100 focus-visible:before:opacity-100 active:translate-y-px active:bg-(--bg-active) ${
                        compact ? "min-h-11 min-w-11 px-3" : "min-h-7 px-1.5"
                      }`}
                      key={key}
                      type="button"
                      onClick={() => {
                    ...
                    >
                      ...
                      {shortcut && shortcut.length > 0 ? (
                        <span className="flex shrink-0 items-center gap-0.5" aria-hidden="true">
                          {shortcut.map((keycap, keycapIndex) => (
                            <kbd
                              className="inline-flex min-h-[1.125rem] min-w-[1.125rem] items-center justify-center whitespace-nowrap rounded-sm border border-(--border-default) bg-(--bg-secondary) px-1 text-center font-sans text-[10px] leading-4 font-medium text-(--workspace-home-muted)"
