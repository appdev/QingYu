...
// Integration tests compress QingYu's debounce; its production timing remains covered by deferred-markdown-change.test.ts.
...
import { readTextSelectionFromView } from "../hooks/useEditorController";
import type {
...
async function renderEditor(
  ...
  const result = render(
    <MarkdownPaper
      ...
      onSaveEditorResources={options.onSaveEditorResources}
...
describe("MarkdownPaper editing", () => {
  ...
  it("passes clipboard and drop origins through the unified resource callback", async () => {
    const onSaveEditorResources = vi.fn<SaveEditorResources>(async (request) => {
      const file = request.origin === "remote" ? null : request.files[0];
      return [{
        alt: file?.name ?? "remote",
        kind: "image",
        src: `assets/${request.origin}.png`
      }];
    });
    const pasted = new File([new Uint8Array([1])], "paste.png", { type: "image/png" });
    const dropped = new File([new Uint8Array([2])], "drop.png", { type: "image/png" });
    const { view } = await renderEditor("", { onSaveEditorResources });

    expect(pasteImage(view, pasted)).toBe(true);
    await waitFor(() => expect(onSaveEditorResources).toHaveBeenCalledWith({
      files: [pasted],
      origin: "clipboard"
    }));

    expect(dropImage(view, dropped)).toBe(true);
    await waitFor(() => expect(onSaveEditorResources).toHaveBeenCalledWith({
      files: [dropped],
      origin: "drop"
    }));
  ...
  it("accepts canonical lowercase file URLs returned by the resource callback", async () => {
    const onSaveEditorResources = vi.fn<SaveEditorResources>().mockResolvedValue([{
      kind: "attachment",
      label: "Reference Doc.pdf",
      src: "file:///Users/example/notes/Reference%20Doc.pdf"
    }]);
    const attachment = new File([new Uint8Array([1])], "Reference Doc.pdf", {
      type: "application/pdf"
    });
    const { container, editor, view } = await renderEditor("", { onSaveEditorResources });

    expect(pasteFile(view, attachment)).toBe(true);

    await waitFor(() => expect(container.querySelector<HTMLAnchorElement>(
      'a[href="file:///Users/example/notes/Reference%20Doc.pdf"]'
    )).toHaveTextContent("Reference Doc.pdf"));
    const serializeMarkdown = editor.action((ctx) => ctx.get(serializerCtx));
    expect(serializeMarkdown(view.state.doc)).toContain(
      "[Reference Doc.pdf](file:///Users/example/notes/Reference%20Doc.pdf)"
    );
  });

  it("rejects dangerous URLs returned by the resource callback", async () => {
    for (const src of [
      "javascript:alert(1)",
      ...
      const attachment = new File([new Uint8Array([1])], "Unsafe.pdf", {
        type: "application/pdf"
      });
      const { container, editor, unmount, view } = await renderEditor("", { onSaveEditorResources });

      expect(pasteFile(view, attachment)).toBe(true);
      await waitFor(() => expect(onSaveEditorResources).toHaveBeenCalled());

      expect(container.querySelector(".ProseMirror a")).not.toBeInTheDocument();
      const serializeMarkdown = editor.action((ctx) => ctx.get(serializerCtx));
      expect(serializeMarkdown(view.state.doc)).not.toContain(src);
      unmount();
    }
  });
  ...
  it("reorders blocks below a table drop target", async () => {
    const tableMarkdown = ["| Name | Role |", "| --- | --- |", "| QingYu | Editor |"].join("\n");
    ...
    const serializeMarkdown = editor.action((ctx) => ctx.get(serializerCtx));
    expect(serializeMarkdown(view.state.doc)).toContain("| QingYu | Editor |\n\nFirst\n\nSecond");
    restoreLayout();
  });
  ...
  it("adds table rows and columns from the hover controls", async () => {
    const initialTable = ["| Field | Value |", "| --- | --- |", "| Name | QingYu |"].join("\n");
  ...
  it("does not notify code block selections as ordinary selection changes", async () => {
    const onTextSelectionChange = vi.fn();
  ...
  it("reads the current text block when no text is selected", async () => {
    ...
    expect(readTextSelectionFromView(view)).toEqual({
      from,
  ...
  it("reads full-document text selections", async () => {
    const { view } = await renderEditor("# Title\n\nFirst paragraph.");
  ...
  it("keeps a selection highlight visible after the editor loses its native selection", async () => {
    ...
    showSelectionHold(view, selection);
    expect(container.querySelector(".ProseMirror .markra-selection-hold")).toHaveTextContent("Original");
  ...
  it("keeps multi-line selection highlights scoped to text inside list items", async () => {
    ...
    showSelectionHold(view, {
      from,
      text: ["First item", "Second item", "Third item"].join("\n"),
      to
    });

    const holds = Array.from(container.querySelectorAll(".ProseMirror .markra-selection-hold"));
  ...
  it("renders links and images from existing markdown", async () => {
    const { container } = await renderEditor(
      "[QingYu](https://example.com)\n\n![QingYu logo](https://example.com/logo.png)"
    );
    ...
    expect(link).toHaveTextContent("QingYu");
    expectLinkIconCount(container, 1);
    expect(image).toBeInTheDocument();
    expect(image).toHaveAttribute("alt", "QingYu logo");
    await settleMarkdownListener();
  });
  ...
  it("keeps normal finalized link clicks editable and opens links with a modifier click", async () => {
    ...
    const modifierCase = await renderEditor("[QingYu](https://example.com)", { openExternalUrl });
    ...
    expect(modifierCase.container.querySelector(".ProseMirror")?.textContent).toBe("QingYu");

    const editCase = await renderEditor("[QingYu](https://example.com)", { openExternalUrl });
    ...
    expect(editCase.container.querySelector(".ProseMirror")?.textContent).toBe("[QingYu](https://example.com)");
    expectActiveSourceLinkLabel(editCase.container, "QingYu");
  ...
  it("expands finalized links into editable markdown source", async () => {
    const { container, editor, view } = await renderEditor("[QingYu](https://example.com) after");
    ...
    expect(container.querySelector(".ProseMirror")?.textContent).toBe("[QingYu](https://example.com) after");
    expectActiveSourceLinkLabel(container, "QingYu");
    ...
    expect(container.querySelector(".ProseMirror")?.textContent).toBe("[QingYu](https://edited.example) after");
    expect(pressEnter(view)).toBe(true);
    ...
    const serializeMarkdown = editor.action((ctx) => ctx.get(serializerCtx));
    expect(serializeMarkdown(view.state.doc)).toContain("[QingYu](https://edited.example)");
  });
  ...
  it("keeps typed link markdown editable and styles the label live", async () => {
    ...
    typeText(view, "[QingYu](https://example.com)");

    expectActiveSourceLinkLabel(container, "QingYu");
    expect(container.querySelector('.ProseMirror a[href="https://example.com"]')).not.toBeInTheDocument();
    expect(container.querySelectorAll(".ProseMirror .markra-live-link-source.markra-md-delimiter")).toHaveLength(2);
    expect(container.querySelector(".ProseMirror")?.textContent).toBe("[QingYu](https://example.com)");
    await settleMarkdownListener();
  });
  ...
  it("keeps typed image markdown editable until it folds into a live preview", async () => {
    ...
    typeText(view, "![QingYu logo](https://example.com/logo.png) after");
    moveCursor(view, findTextPosition(view, "QingYu logo", "QingYu logo".length));
    ...
    expect(container.querySelector(".ProseMirror")?.textContent).toBe(
      "![QingYu logo](https://example.com/logo.png) after"
    );
  ...
  it("folds raw link markdown when the cursor leaves the source", async () => {
    ...
    insertTextDirectly(view, "[QingYu](https://example.com) after");
    moveCursor(view, findTextPosition(view, "QingYu", "QingYu".length));
    ...
    expectLiveLink(container, "QingYu");
    expectLinkIconCount(container, 1);
  ...
  it("folds raw link markdown after clicking editor blank space", async () => {
    ...
    insertTextDirectly(view, "[QingYu](https://example.com)");
    moveCursor(view, findTextPosition(view, "QingYu", "QingYu".length));
  ...
  it("folds typed image markdown after clicking paper blank space", async () => {
    ...
    insertTextDirectly(view, "![QingYu logo](https://example.com/logo.png)");
    moveCursor(view, findTextPosition(view, "QingYu logo", "QingYu logo".length));
  ...
  it("finalizes raw link and image markdown on enter", async () => {
    ...
    insertTextDirectly(linkCase.view, "[QingYu](https://example.com)");

    expect(linkCase.container.querySelector(".ProseMirror")?.textContent).toBe("[QingYu](https://example.com)");
    ...
    expect(link).toHaveTextContent("QingYu");
    expect(linkCase.container.querySelector(".ProseMirror")?.textContent).toBe("QingYu");
    ...
    insertTextDirectly(imageCase.view, "![QingYu logo](https://example.com/logo.png)");
