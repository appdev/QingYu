...
function insertMarkdownImageReferences(
  ...
  const safeImages = images.filter((savedImage) => isSafeEditorResourceUrl(savedImage.src));
  if (!safeImages.length) return;

  const range = emptyTopLevelParagraphSelectionRange(view, selection) ?? selection;
  const fragment = Fragment.fromArray(
    safeImages.map((savedImage) =>
      image.create({
...
function insertMarkdownLinkReferences(
  ...
  const safeLinks = links.filter((reference) => isSafeEditorResourceUrl(reference.href));
  if (!safeLinks.length) return;

  const nodes: ProseNode[] = [];

  safeLinks.forEach((reference, index) => {
...
export function readTextSelectionFromView(view: EditorView): EditorTextSelection {
...
export function useEditorController() {
  ...
  const getSelection = useCallback((): EditorTextSelection | null => {
    try {
      const view = editorRef.current?.action((ctx) => ctx.get(editorViewCtx));
      if (!view) return null;

      return readTextSelectionFromView(view);
    } catch {
      return null;
    }
  }, []);
  ...
  const holdSelection = useCallback((selection: EditorTextSelection) => {
    try {
      ...
      // Losing this decoration should not interrupt the selection flow.
    }
  }, []);
  ...
  const clearSelection = useCallback(() => {
    try {
      ...
      // The editor may be unavailable while the selection surface is closing.
    }
  }, []);
