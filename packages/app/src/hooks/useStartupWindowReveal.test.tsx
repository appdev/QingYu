...
describe("useStartupWindowReveal", () => {
  ...
  it("does not schedule or reveal the native window while the capability is disabled", async () => {
    const revealWindow = vi.fn().mockResolvedValue(undefined);
    render(
      <StartupWindowRevealTest
        enabled={false}
  ...
  it("cancels a pending fallback reveal when the capability becomes disabled", async () => {
    const revealWindow = vi.fn().mockResolvedValue(undefined);
    const view = render(
      <StartupWindowRevealTest
        fallbackMs={20}
