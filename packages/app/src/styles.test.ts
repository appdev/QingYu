...
function readSeededThemeStyles(themeId: typeof seededThemeIds[number]) {
  const fixturePath = themeId === "drake-light" || themeId === "drake-ayu"
    ...
    : `${themeId}.css`;
  return readFileSync(
    `${process.cwd()}/../../apps/desktop/src-tauri/themes/third-party/${fixturePath}`,
    "utf8"
  );
}

describe("editor stylesheet", () => {
  it("scopes Compact safe areas, keyboard avoidance, and touch behavior to the Compact root", () => {
    const styles = readFileSync(`${process.cwd()}/src/styles.css`, "utf8");
    const compactRuleStart = styles.indexOf('.compact-app-shell[data-compact="true"] {');
    const compactRuleEnd = styles.indexOf("  }", compactRuleStart);
    const compactRule = styles.slice(compactRuleStart, compactRuleEnd);
    const compactEditorSpacerStart = styles.indexOf(
      '.compact-app-shell[data-compact="true"] .paper-scroll::after {'
    );
    const compactEditorSpacerEnd = styles.indexOf("  }", compactEditorSpacerStart);
    const compactEditorSpacerRule = styles.slice(compactEditorSpacerStart, compactEditorSpacerEnd);
    const globalRootRuleStart = styles.indexOf("  html,\n  body,\n  #root {");
    const globalRootRuleEnd = styles.indexOf("  }", globalRootRuleStart);
    const globalRootRule = styles.slice(globalRootRuleStart, globalRootRuleEnd);

    expect(compactRuleStart).toBeGreaterThanOrEqual(0);
    expect(compactRule).toContain("--compact-safe-area-top: env(safe-area-inset-top, 0px)");
    expect(compactRule).toContain("--compact-safe-area-bottom: env(safe-area-inset-bottom, 0px)");
    expect(compactRule).toContain("--compact-keyboard-inset: 0px");
    expect(compactRule).toContain(
      "--compact-bottom-inset: max(var(--compact-safe-area-bottom), var(--compact-keyboard-inset))"
    );
    expect(compactRule).toContain("overscroll-behavior: none");
    expect(styles).toContain('.compact-app-shell[data-compact="true"] [data-compact-scroll="vertical"]');
    expect(styles).toContain("overscroll-behavior-y: contain");
    expect(styles).toContain("touch-action: pan-y");
    expect(compactEditorSpacerStart).toBeGreaterThanOrEqual(0);
    expect(compactEditorSpacerRule).toContain("display: block");
    expect(compactEditorSpacerRule).toContain(
      "block-size: calc(3.25rem + var(--compact-bottom-inset))"
    );
    expect(compactEditorSpacerRule).toContain('content: ""');
    expect(styles).toContain('.compact-app-shell[data-compact="true"] [data-compact-scroll="horizontal"]');
    expect(styles).toContain("overscroll-behavior-x: contain");
    expect(styles).toContain("touch-action: pan-x");
    expect(styles).toContain('.compact-app-shell[data-compact="true"] :where(button, [role="button"]):focus-visible');
    expect(styles).toContain('.compact-app-shell[data-compact="true"] :where(button, [role="button"]):active:not(:disabled)');
    expect(globalRootRule).toContain("@apply m-0 h-full w-full min-w-190 overscroll-none");
    expect(globalRootRule).not.toContain("overflow: hidden");
    expect(globalRootRule).not.toContain("--compact-keyboard-inset");
  });

  it("reduces motion only inside Compact mode", () => {
    const styles = readFileSync(`${process.cwd()}/src/styles.css`, "utf8");
    ...
    expect(reducedMotionStyles).toContain('.compact-app-shell[data-compact="true"]');
    expect(reducedMotionStyles).toContain("animation-duration: 0.01ms");
    expect(reducedMotionStyles).toContain("transition-duration: 0.01ms");
    expect(reducedMotionStyles).toContain("scroll-behavior: auto");
    expect(reducedMotionStyles).not.toContain("html,");
    expect(reducedMotionStyles).not.toContain("body,");
  });
  ...
  it("disables transitions while a theme change is being painted", () => {
    const styles = readFileSync(`${process.cwd()}/src/styles.css`, "utf8");

    expect(styles).toContain('html[data-theme-transition="suspended"]');
    expect(styles).toContain('html[data-theme-transition="suspended"] *');
    expect(styles).toContain('html[data-theme-transition="suspended"] *::before');
    expect(styles).toContain('html[data-theme-transition="suspended"] *::after');
  ...
  it("uses theme-aware custom text cursors for visual editor text surfaces", () => {
    ...
    expect(styles).toContain('[data-theme-appearance="dark"] .markdown-paper');
  ...
  it("maps QingYu document typography onto CodeMirror without styling its widget buffers as content images", () => {
    const styles = readFileSync(`${process.cwd()}/src/styles.css`, "utf8");
  ...
  it("adds GitHub heading dividers and sizes in GitHub editor themes", () => {
    const githubStyles = readSeededThemeStyles("github");
    const githubDarkStyles = readSeededThemeStyles("github-dark");

    for (const styles of [githubStyles, githubDarkStyles]) {
      expect(styles).toContain("h1");
      expect(styles).toContain("h2");
      expect(styles).toContain("border-bottom: 1px solid var(--editor-border);");
      expect(styles).toContain("padding-bottom: 0.3em;");
      expect(styles).toContain("font-size: 2em;");
      expect(styles).toContain("font-size: 1.5em;");
  ...
  it("suppresses editor selection chrome while document search is open", () => {
    ...
    const searchChromeEnd = styles.indexOf("@media (prefers-reduced-motion: reduce)", searchChromeStart);
  ...
  it("keeps selection holds from drawing connected borders across wrapped lines", () => {
    const styles = readFileSync(`${process.cwd()}/src/styles.css`, "utf8");
    const selectionHoldStart = styles.indexOf(".markdown-paper .markra-selection-hold {");
    const selectionHoldEnd = styles.indexOf(".markdown-paper code {", selectionHoldStart);
  ...
  it("keeps Markdown markers readable in dark themes", () => {
    ...
    const darkSyntaxStart = styles.indexOf(
      '[data-theme-appearance="dark"] .markdown-paper {',
    );
    ...
    const darkSourceSyntaxStart = styles.indexOf(
      '[data-theme-appearance="dark"] .markdown-source-paper {',
    );
  ...
  it("defines Typora-style editor themes and code block copy controls", () => {
    ...
    const paperEnd = styles.indexOf('[data-theme-appearance="dark"] .markdown-paper', paperStart);
    const paperStyles = styles.slice(paperStart, paperEnd);

    for (const theme of seededThemeIds) {
      const themeStyles = readSeededThemeStyles(theme);
      expect(themeStyles).toContain(`.markdown-paper[data-editor-theme="${theme}"]`);
      expect(themeStyles).toContain(`[data-theme="${theme}"]`);
    }

    expect(paperStart).toBeGreaterThanOrEqual(0);
    expect(paperEnd).toBeGreaterThan(paperStart);
    expect(paperStyles).not.toContain("background: var(--editor-paper-bg)");
    expect(styles).toContain('[data-theme-appearance="dark"] .markdown-paper');
    expect(readSeededThemeStyles("night")).not.toContain('.markdown-paper[data-editor-theme="night"] .ProseMirror');
  ...
  it("keeps Drake packages semantic, self-contained, and scoped to QingYu selectors", () => {
    for (const theme of ["drake-light", "drake-ayu"] as const) {
      const styles = readSeededThemeStyles(theme);

      expect(styles.match(/@font-face/g)).toHaveLength(4);
      ...
      expect(styles).toContain(".markra-live-mark-highlight");
      expect(styles).toContain(".markra-task-list-checkbox");
      expect(styles).toContain(".markra-mermaid-render");
      expect(styles).toContain(".markra-code-block");
  ...
  it("keeps GitHub editor tables aligned with Primer Markdown spacing", () => {
    const styles = readSeededThemeStyles("github");

    expect(styles).toContain('.markdown-paper[data-editor-theme="github"] .tableWrapper');
    expect(styles).toContain("margin-block: 1rem;");
    expect(styles).toContain('.markdown-paper[data-editor-theme="github"] .markra-table-controls-wrapper');
    expect(styles).toContain("padding: 0;");
    expect(styles).toContain('.markdown-paper[data-editor-theme="github"] table');
    expect(styles).toContain("font-size: 1em;");
    expect(styles).toContain("font-variant: tabular-nums;");
    expect(styles).toContain("line-height: 1.5;");
    expect(styles).toContain("padding: 6px 13px;");
    expect(styles).toContain("border-color: var(--editor-border);");
    expect(styles).toContain("font-weight: 600;");
  });

  it("keeps GitHub Dark and One themes aligned with their source palettes", () => {
    const githubDarkStyles = readSeededThemeStyles("github-dark");
    const oneDarkStyles = readSeededThemeStyles("one-dark");
    const oneLightStyles = readSeededThemeStyles("one-light");
    const oneDarkProStyles = readSeededThemeStyles("one-dark-pro");
