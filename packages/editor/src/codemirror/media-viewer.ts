...
function focusableMediaViewerElements(dialog: HTMLElement) {
  return Array.from(dialog.querySelectorAll<HTMLElement>(focusableSelector))
    .filter((element) => !element.hidden && element.ariaHidden !== "true");
}

function isolateMediaViewerBackground(dialog: HTMLElement) {
  const inertedElements: Element[] = [];
  let foreground: Element = dialog;
  while (foreground !== dialog.ownerDocument.body) {
...
export function openMediaViewer({
  ...
  let translateX = 0;
  let translateY = 0;
  let restoreBackground: (() => unknown) | null = null;
  let drag: {
    originX: number;
    originY: number;
    pointerId: number;
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    if (event.key === "Tab") {
      const focusable = focusableMediaViewerElements(dialog);
  ...
  content.addEventListener("keydown", (event) => {
    ...
    else if (event.key === "ArrowRight") translateX -= distance;
    else if (event.key === "ArrowUp") translateY += distance;
    else if (event.key === "ArrowDown") translateY -= distance;
  ...
  dialog.append(panel);
  mount.append(dialog);
  restoreBackground = isolateMediaViewerBackground(dialog);
