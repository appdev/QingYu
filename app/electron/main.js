...
const managedDevKernel = isDevEnv && process.env.QINGYU_DEV_MANAGED_KERNEL === "1";
...
if (isDevEnv && process.defaultApp && process.argv.length >= 2) {
    ...
    if (process.platform === "win32") {
        app.removeAsDefaultProtocolClient("qingyu", process.execPath, [mainScript]);
        app.setAsDefaultProtocolClient("qingyu", process.execPath, [mainScript]);
    } else {
        app.setAsDefaultProtocolClient("qingyu");
    }
} else {
    app.setAsDefaultProtocolClient("qingyu");
}
...
for (let i = argStart; i < process.argv.length; i++) {
    ...
    if (arg.startsWith("--workspace=") || arg.startsWith("--openAsHidden") || arg.startsWith("--port=") || arg.startsWith("--safe-mode=") || arg.startsWith("--lang=") || arg.startsWith("qingyu://")) {
        // 跳过内置参数
...
try {
    ...
    console.error(e);
    require("electron").dialog.showErrorBox("创建配置目录失败 Failed to create config directory", "轻语需要在用户家目录下创建配置文件夹（~/.config/qingyu），请确保该路径具有写入权限。\n\nQingYu needs to create a configuration folder (~/.config/qingyu) in the user's home directory. Please make sure that the path has write permissions.");
    app.exit();
}
...
const getWorkspaceByWebContentsId = (webContentsId) => {
    return workspaces.find((item) => item.webContentsId === webContentsId);
};

const getWorkspaceByWindow = (window) => {
    if (!window || window.isDestroyed()) {
        return;
    }
    const directWorkspace = getWorkspaceByWebContentsId(window.webContents.id);
    if (directWorkspace) {
        return directWorkspace;
    }
    try {
        const port = new URL(window.getURL()).port;
        return workspaces.find((item) => item.port.toString() === port);
    } catch (e) {
        return;
    }
};

const dispatchNativeMenuCommand = (command) => {
    if (!NATIVE_MENU_COMMANDS.has(command)) {
        return;
    }
    const focusedWindow = BrowserWindow.getFocusedWindow();
    const workspace = getWorkspaceByWindow(focusedWindow);
    if (!workspace || !workspace.browserWindow || workspace.browserWindow.isDestroyed()) {
        return;
    }
    workspace.browserWindow.webContents.send("siyuan-native-menu-command", command);
};

const installApplicationMenu = (preferredWindow) => {
    const productName = "QingYu";
    let state = createDefaultNativeMenuState();
    const workspace = getWorkspaceByWindow(preferredWindow);
    if (workspace) {
        state = nativeMenuStates.get(workspace.webContentsId) || state;
    }
    const template = createApplicationMenuTemplate({
        platform: process.platform,
        productName,
        state,
        dispatch: dispatchNativeMenuCommand,
        hotKey2Electron,
    });
...
app.on("browser-window-focus", (event, focusedWindow) => {
    ...
    if (process.platform === "darwin") {
        installApplicationMenu(focusedWindow);
    }
});
...
const exitApp = (port, errorWindowId) => {
    ...
    if (workspace) {
        nativeMenuStates.delete(workspace.webContentsId);
...
const resetSystemShutdown = (ports) => {
    ...
    writeLog("system shutdown canceled because QingYu failed to exit gracefully [ports=" + ports.join(",") + "]");
    ports.forEach((port) => {
...
if (process.platform === "win32") {
    // Windows 关机、重启或注销时取消本次会话结束，等待内核安全退出后再关闭轻语。
...
const initMainWindow = (currentKernelPort = kernelPort) => {
    ...
    currentWindow.webContents.userAgent = "QingYu/" + appVer + " Electron " + currentWindow.webContents.userAgent;
    ...
    currentWindow.webContents.on("did-finish-load", () => {
        let siyuanOpenURL = process.argv.find((arg) => arg.startsWith("qingyu://"));
        if (siyuanOpenURL) {
    ...
    if (process.platform === "darwin") {
        ...
        currentWindow.webContents.once("destroyed", () => {
            nativeMenuStates.delete(currentWebContentsId);
        });
    } else {
        installApplicationMenu(currentWindow);
    }
...
const initKernel = (workspace, port, lang, safeMode) => {
    return new Promise(async (resolve) => {
        ...
        if (!fs.existsSync(kernelPath)) {
            showErrorWindow("内核程序丢失", "Kernel program is missing", `<div>内核程序丢失，请重新安装轻语，并将轻语内核程序加入杀毒软件信任列表。</div><div>The kernel program is not found, please reinstall QingYu and add QingYu Kernel prgram into the trust list of your antivirus software.</div><div><i>${kernelPath}</i></div>`);
            bootWindow.destroy();
            resolve(false);
            return;
        }
        ...
        writeLog(cmd);
        if (shouldSpawnKernel({
            development: isDevEnv,
            managed: managedDevKernel,
                ...
                if (0 !== code && !expectedExit) {
                    ...
                    switch (code) {
                        ...
                        case 24: // 工作空间已被锁定，尝试切换到第一个打开的工作空间
                            ...
                            errorWindowId = showErrorWindow("工作空间已被锁定", "The workspace is locked", "<div>该工作空间正在被使用，请尝试在任务管理器中结束 QingYu-Kernel 进程或者重启操作系统后再启动轻语。</div><div>The workspace is being used, please try to end the QingYu-Kernel process in the task manager or restart the operating system and then start QingYu.</div>");
                            break;
                        case 25:
                        ...
                        case 0:
                            break;
                        default:
                            errorWindowId = showErrorWindow("内核因未知原因退出", "The kernel exited for unknown reasons", `<div>轻语内核因未知原因退出 [code=${code}]，请尝试重启操作系统后再启动轻语。如果该问题依然发生，请检查杀毒软件是否阻止轻语内核启动。</div><div>QingYu Kernel exited for unknown reasons [code=${code}], please try to reboot your operating system and then start QingYu again. If occurs this problem still, please check your anti-virus software whether kill the QingYu Kernel.</div>`);
                            break;
                    }
        ...
        for (; ;) {
            try {
                ...
                if (14 < ++count) {
                    ...
                    showErrorWindow("获取内核服务端口失败", "Failed to Obtain Kernel Service Port", "<div>获取内核服务端口失败，请确保程序拥有网络权限并不受防火墙和杀毒软件阻止。</div><div>Failed to obtain kernel service port. Please ensure QingYu has network permissions and is not blocked by firewalls or antivirus software.</div>");
                    bootWindow.destroy();
                    resolve(false);
                    return;
                }
                ...
                while (!progressing) {
                    if (Date.now() - bootShowStart > bootTimeout) {
                        ...
                        showErrorWindow("启动超时", "Boot timeout",
                            "<div>内核启动超时，请查看 工作空间/temp/siyuan.log 获取详细报错信息，或尝试重启轻语。</div>" +
                            "<div>Kernel boot timed out. Please check workspace/temp/siyuan.log for details, or try restarting QingYu.</div>");
                        requestKernelExit(currentKernelPort);
                        bootWindow.destroy();
                        resolve(false);
                        progressing = true;
...
app.whenReady().then(() => {
    if (process.platform === "darwin" && app.dock) {
        app.dock.setIcon(path.join(appDir, "stage", "icon-large.png"));
    }
    ...
    ipcMain.on("siyuan-native-menu-state", (event, value) => {
        if (process.platform !== "darwin" || !getWorkspaceByWebContentsId(event.sender.id)) {
            return;
        }
        const state = sanitizeNativeMenuState(value);
        if (!state) {
            return;
        }
        nativeMenuStates.set(event.sender.id, state);
    ...
    ipcMain.on("siyuan-export-newwindow", (event, data) => {
        ...
        printWin.center();
        printWin.webContents.userAgent = "QingYu/" + appVer + " Electron " + printWin.webContents.userAgent;
    ...
    ipcMain.handle("siyuan-init", async (event, data) => {
        ...
        if (workspaceItem) {
            ...
            if ("win32" === process.platform || "linux" === process.platform) {
                ...
                tray.setToolTip(`${path.basename(data.workspaceDir)} - QingYu v${appVer}`);
                const mainWindow = getWindowByContentId(event.sender.id);
                if (!mainWindow || mainWindow.isDestroyed()) {
                    tray.destroy();
                    tray = undefined;
...
app.on("second-instance", (event, argv) => {
    ...
    const siyuanURL = argv.find((arg) => arg.startsWith("qingyu://"));
    workspaces.forEach(item => {
