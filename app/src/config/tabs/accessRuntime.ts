...
export const saveMCP = (overrides: Partial<Config.IAPI["mcp"]>) => {
    fetchPost("/api/system/setMCP", overrides, (response: IWebSocketData) => {
        if (response.code === 0) {
            window.siyuan.config.api.mcp = response.data;
        }
    });
};
