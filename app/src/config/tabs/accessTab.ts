...
/// #endif
import {isInMobileApp, saveExportFile, writeText} from "../../protyle/util/compatibility";
import {openByMobile} from "../../editor/openLink";
import {genConfigItemMainHtml} from "../render/fragments";
import {renderPublishAuthAccounts, saveMCP, savePublish, sendAccessSetting, updatePublishConfig} from "./accessRuntime";
...
const getMCPURL = () => new URL("/mcp", window.location.origin).toString();

const getMCPConfig = () => JSON.stringify({
    mcpServers: {
        qingyu: {
...
const copyMCPText = (text: string) => {
    writeText(text);
    showMessage(window.siyuan.languages.copied);
};

const registerMCPGroup = (tab: SettingTabBuilder) => {
    const group = tab.group("mcp", "MCP Server");
    group.switch("api.mcp.enabled", {
        title: window.siyuan.languages.enable,
        desc: window.siyuan.languages.mcpServerTip,
        save: (value) => saveMCP({enabled: Boolean(value)}),
    });
    group.select("api.mcp.permission", {
        title: window.siyuan.languages.mcpPermission,
        desc: window.siyuan.languages.mcpPermissionTip,
        options: [
            {value: "readOnly", label: window.siyuan.languages.mcpReadOnly},
            {value: "fullAccess", label: window.siyuan.languages.mcpFullAccess},
        ],
        readConfig: () => window.siyuan.config.api.mcp.readOnly ? "readOnly" : "fullAccess",
        save: (value) => saveMCP({readOnly: value === "readOnly"}),
    });
    group.button({
        id: "copyMCPConfig",
        title: "MCP JSON",
        desc: window.siyuan.languages.mcpConnectionTip,
        label: window.siyuan.languages.copy,
        icon: "iconCopy",
        afterMount: (root) => {
            root.querySelector("#copyMCPConfig")?.addEventListener("click", () => copyMCPText(getMCPConfig()));
        },
    });
    group.button({
        id: "copyMCPPrompt",
        ...
        icon: "iconCopy",
        afterMount: (root) => {
            root.querySelector("#copyMCPPrompt")?.addEventListener("click", () => {
                const prompt = window.siyuan.languages.mcpAIPrompt
                    .replace("${url}", getMCPURL())
                    .replace("${token}", window.siyuan.config.api.token);
                copyMCPText(prompt);
            });
        },
    });
