...
import {isAppBlockURI, isLocalPath, parseSiYuanUriInfo} from "../util/pathName";
...
const hidePopover = (event: MouseEvent & { path: HTMLElement[] }) => {
    ...
    if (!popoverTargetElement && linkElement && isAppBlockURI(linkElement.getAttribute("data-href"))) {
...
const getTarget = (event: MouseEvent & { target: HTMLElement }, aElement: false | HTMLElement) => {
    ...
    if (!popoverTargetElement && aElement) {
        if (isAppBlockURI(aElement.getAttribute("data-href")) && aElement.getAttribute("prevent-popover") !== "true") {
            ...
            if (textElement && textElement.dataset.type === "url" && isAppBlockURI(textElement.dataset.href)) {
