...
function changesStayAfter(
  change: Pick<Transaction, "changes" | "docChanged" | "startState" | "state">,
  position: number,
...
export function transactionChangesStayAfter(
  ...
  return changesStayAfter(transaction, position, insertedMayAffectSyntax);
}
...
export function updateChangesStayAfter(
  ...
  return !update.focusChanged && changesStayAfter(update, position, insertedMayAffectSyntax);
}
