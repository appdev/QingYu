...
import { createTableControls, defaultTableLabels, type TableControlLabels } from "./table-controls";
import { clearTableSelection, activeTableCell, placeTableCellCaret, tableCellCaretOffset, createTableCaretHost, tableCaretPlaceholder } from "./table-selection";
import { transformHtmlTable, htmlTableAlignment, htmlTableWidthMode, type HtmlTableAction } from "../html-table-actions";
import { isolateHistory, redo, undo } from "@codemirror/commands";
import { WidgetType, type EditorView } from "@codemirror/view";
import { sanitizeRawHtml, type RawHtmlSanitizeOptions } from "../raw-html-sanitize";
import {
...
import { attachHtmlColumnResizers } from "./html-table-resize";
  ...
  private render(view: EditorView, root: HTMLElement) {
    ...
    rendered.forEach((table, index) => {
      ...
      this.runtime.controls.push(createTableControls(wrapper, table, {
        ...
        extras: [
          { label: this.labels.mergeCells, action: "merge", run: merge, icon: createSiyuanMarkdownIcon(document, "table", "markra-table-control-icon"), enabled: () => !this.runtime.composing && Boolean(selection()?.mergeable) },
          { label: this.labels.splitCell, action: "split", run: split, icon: createSiyuanMarkdownIcon(document, "remove", "markra-table-control-icon"), enabled: () => {
