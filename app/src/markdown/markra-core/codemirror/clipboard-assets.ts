...
function savedImage(resource: SavedEditorResource): resource is Extract<SavedEditorResource, {kind: "image"}> {
...
export interface CodeMirrorClipboardAssetsPluginOptions {
  ...
  saveResources?: SaveEditorResources;
  uploadLabel?: string;
}
...
function attachmentSaver(
  options: CodeMirrorClipboardAssetsPluginOptions,
  origin: "clipboard" | "drop",
): SaveClipboardAttachment | undefined {
  if (options.saveResources) {
    return async (attachment) => {
      const resources = await options.saveResources?.({ files: [attachment], origin }) ?? [];
      const saved = resources.find(savedAttachment);
      return saved ? { label: saved.label, src: saved.src } : null;
    };
  }
  return options.saveAttachment;
}

function remoteImageSaver(
  options: CodeMirrorClipboardAssetsPluginOptions,
): SaveRemoteClipboardImage | undefined {
  if (options.saveResources) {
    return async (image) => {
      const resources = await options.saveResources?.({ origin: "remote", urls: [image.src] }) ?? [];
      const saved = resources.find(savedImage);
      return saved ? { alt: saved.alt, src: saved.src } : null;
    };
  }
  return options.saveRemoteImage;
}
