import { createSiyuanMarkdownIcon } from "../shared";
import { popoverPosition } from "../shared";
...
export function createTableControls(wrapper: HTMLElement, table: HTMLTableElement, options: TableControlOptions) {
  ...
  const remove = createTableControl(document, "markra-table-delete-table", options.labels.deleteTable, options.deleteTable, createSiyuanMarkdownIcon(document, "trash", "markra-table-control-icon"));
  toolbar.append(sizeControls, ...alignments, width, ...extraButtons, remove);
  const column = createTableControl(document, "markra-table-add-column", options.labels.addColumnRight, options.addColumn, createSiyuanMarkdownIcon(document, "add", "markra-table-control-icon"));
  const row = createTableControl(document, "markra-table-add-row", options.labels.addRowBelow, options.addRow, createSiyuanMarkdownIcon(document, "add", "markra-table-control-icon"));
  let hoveredColumn = 0, hoveredRow = 0;
  const deleteColumn = createTableControl(document, "markra-table-delete-control markra-table-delete-column", options.labels.deleteColumn, () => options.deleteColumn(hoveredColumn), createSiyuanMarkdownIcon(document, "remove", "markra-table-control-icon"));
  const deleteRow = createTableControl(document, "markra-table-delete-control markra-table-delete-row", options.labels.deleteRow, () => options.deleteRow(hoveredRow), createSiyuanMarkdownIcon(document, "remove", "markra-table-control-icon"));
