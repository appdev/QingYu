...
import { markraHighlight } from "./highlight";
import { resolveSafeImageSource } from "./image";
import {
...
function mathRangeCrossingChild(
  ...
  // TeX 标点可能被识别为 Markdown，跨语法节点的公式必须保持完整。
...
function serializeNode(node: Node): string {
  ...
  // 公式只序列化原始 Markdown，避免将 KaTeX 的渲染文本写回单元格。
