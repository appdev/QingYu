...
function formattingMarkers(node: SyntaxTreeNode, spec: FormattingSpec) {
  ...
  while (child) {
    if (child.name === spec.markerNode) markers.push({from: child.from, to: child.to});
...
function formattingNormalization(state: EditorState, range: SelectionRange, spec: FormattingSpec) {
  ...
  visitFormattingNodes(state, spec, range.from, range.to, (node) => {
    ...
    if (opening && closing && opening.to <= range.to && closing.from >= range.from) {
      from = Math.min(from, node.from);
      to = Math.max(to, node.to);
      changes.push({from: opening.from, to: opening.to}, {from: closing.from, to: closing.to});
    }
  });
