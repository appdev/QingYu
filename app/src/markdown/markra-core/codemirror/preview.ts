...
const HEADING_CLASSES: Readonly<Record<string, string>> = {
  ...
  ATXHeading2: "cm-markra-h2 h2",
  ATXHeading3: "cm-markra-h3 h3",
  ATXHeading4: "cm-markra-h4 h4",
  ATXHeading5: "cm-markra-h5 h5",
  ATXHeading6: "cm-markra-h6 h6",
  SetextHeading1: "cm-markra-h1 h1",
  SetextHeading2: "cm-markra-h2 h2",
};
...
function pushHiddenRange(
  ...
  // Splitting here keeps custom Markdown parsers safe to compose with QingYu.
  while (cursor < to) {
      ...
      if (linkLike && linkRevealed) {
        ...
        ranges.push(
          Decoration.mark({
            class: inlineClass,
            attributes: {
              "data-type": INLINE_TYPES[node.name] ?? (standaloneUrl ? "a" : ""),
              ...(linkHref
                ? {
                    draggable: "false",
                    href: linkHref,
