...
/// #if !MOBILE
import {openFileById, updateBacklink} from "../editor/util";
...
import {getAppBlockURI, isAppBlockURI, isEncryptedBox, parseSiYuanUriInfo} from "../util/pathName";
...
export const refMenu = (protyle: IProtyle, element: HTMLElement) => {
    ...
    if (!protyle.disabled) {
        ...
        submenu = submenu.concat([{
            ...
            iconHTML: "",
            click() {
                element.outerHTML = `<span data-type="a" data-href="${getAppBlockURI(element.getAttribute("data-id"))}">${element.innerHTML}</span><wbr>`;
                nodeElement.setAttribute("updated", dayjs().format("YYYYMMDDHHmmss"));
                updateTransaction(protyle, nodeElement, oldHTML);
                focusByWbr(nodeElement, protyle.toolbar.range);
                oldHTML = nodeElement.outerHTML;
...
export const zoomOut = (options: {
    ...
    fetchPost("/api/filetree/getDoc", getDocParam, async (getResponse) => {
        ...
        if (options.protyle.model) {
            ...
            updateBacklink(allModels, options.protyle);
        }
        /// #endif
    });
};
        ...
        if (isAppBlockURI(linkAddress)) {
            window.siyuan.menus.menu.append(new MenuItem({
                ...
                click() {
                    ...
                    linkElement.setAttribute("data-id", parseSiYuanUriInfo(inputElements[0].value)?.id || "");
