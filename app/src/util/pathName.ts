...
/**
 * Check if the given URI uses the QingYu or legacy SiYuan protocol.
...
export const isAppBlockURI = (uri?: string | null): boolean => {
    return Boolean(uri && (uri.startsWith("qingyu://blocks/") || uri.startsWith("siyuan://blocks/")));
};

export const getAppBlockURI = (id: string): string => `qingyu://blocks/${id}`;

/**
 * Parse qingyu://blocks/20221031001313-rk7sd0e?focus=1&fullscreen=1 or its legacy SiYuan form.
 ...
 * 让内核走加密笔记本版本（查加密 blocktree + content db）。
 */
