...
const processSiYuanUriPlugins = (app: App, uriObj: URL): boolean => {
    ...
    if (plugin) {
        // qingyu://plugins/plugin-name/foo?bar=baz
        plugin.eventBus.emit("open-siyuan-url-plugin", { url: uriObj.href });
    } else {
        if (!app.plugins.some(item => item.models[pluginNameOrTabType])) {
            return false;
        }
        // qingyu://plugins/plugin-samplecustom_tab?title=自定义页签&icon=iconFace&data={"text": "This is the custom plugin tab I opened via protocol."}
        /// #if !MOBILE
...
const processSiYuanUriBazaar = (app: App, uriObj: URL): boolean => {
    ...
    switch (target) {
        ...
        case "readme-installed": {
            // qingyu://bazaar/plugins/plugin-sample/readme
            // qingyu://bazaar/plugins/plugin-sample/readme-installed
