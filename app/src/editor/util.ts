...
import {clearOB} from "../layout/dock/util";
...
export const openMarkdownFile = (app: App, notebookId: string, path: string, name: string, position?: string, removeCurrentTab = true) => {
    return openFile({
        app,
        markdown: {notebookId, path, name},
        position,
        removeCurrentTab,
    });
};
...
export const openFile = async (options: IOpenFileOptions) => {
    ...
    if (options.markdown) {
        ...
        if (markdownEditor) {
            ...
            options.afterOpen?.(markdownEditor);
...
const switchEditor = (editor: Editor, options: IOpenFileOptions, allModels: IModels) => {
    ...
    if ((!nodeElement || nodeElement?.clientHeight === 0) && options.id !== options.rootID) {
        ...
        fetchPost("/api/filetree/getDoc", getDocParam, getResponse => {
            ...
            updateBacklink(allModels, editor.editor.protyle);
        });
    } else {
    ...
    if (activeElement) {
        ...
        if (tab instanceof Tab && tab.model instanceof Editor) {
            if (tab.model.editor.protyle.block.rootID === blockId ||
                tab.model.editor.protyle.block.parentID === blockId ||  // updateBacklink 时会传入 parentID
...
export const updateBacklink = (models: IModels, protyle: IProtyle) => {
    // https://ld246.com/article/1637636106054/comment/1641485541929#comments
