...
const JSONToDock = (json: any, app: App) => {
    [json.left, json.right, json.bottom].forEach((dock: Config.IUILayoutDock) => {
        dock.data = dock.data
            .map(group => group.filter(item => !["graph", "globalGraph"].includes(item.type)))
...
export const JSONToCenter = (
    ...
    } else if ((json as {instance: string}).instance === "Graph") {
        // 升级时移除旧版本保存的关系图页签。
        removedTabs.push(layout as Tab);
        return;
...
export const addResize = (obj: Layout | Wnd, after = true) => {
    ...
    const getMinSize = (element: HTMLElement) => {
        ...
        Array.from(element.querySelectorAll(".file-tree")).find((item) => {
            if (item.classList.contains("sy__backlink")) {
