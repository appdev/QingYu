...
Function AppendInstallLog
    ...
    ClearErrors
    FileOpen $7 "$TEMP\QingYu-install.log" a
...
!macro preInit
    ${IfNot} ${AtLeastWin10}
        ...
        MessageBox MB_ICONEXCLAMATION "非常抱歉，轻语无法在低于 Windows 10 的系统上进行安装$\n$\n\
            Sorry, QingYu cannot be installed on systems below Windows 10$\n"
        Quit
    ${EndIf}
    ...
    Push $R8
    Push $R7
    nsExec::Exec 'TASKKILL /F /IM "QingYu.exe"'
    Pop $R8
    nsExec::Exec 'TASKKILL /F /IM "QingYu-Kernel.exe"'
    Pop $R7
...
!macro customInstall
    ...
    nsExec::ExecToLog 'cmd /c mklink /H "$INSTDIR\resources\kernel\qingyu.exe" "$INSTDIR\resources\kernel\QingYu-Kernel.exe" 2>nul || ver>nul'
    ${If} $installMode == "all"
...
!macro customUnInstall
    ${IfNot} ${isUpdated}
        IfFileExists "$PROFILE\.config\qingyu\*.*" 0 skipConfigDelete
            MessageBox MB_YESNO "是否需要彻底删除全局配置（$PROFILE\.config\qingyu\）？$\n$\n\
                Do you want to delete the global configuration ($PROFILE\.config\qingyu\)?$\n" \
                /SD IDYES IDYES AcceptedRMConf IDNO SkippedRMConf
                AcceptedRMConf:
                    RMDir /r "$PROFILE\.config\qingyu\"
                SkippedRMConf:
        skipConfigDelete:
    ${EndIf}

    ${IfNot} ${isUpdated}
        IfFileExists "$PROFILE\QingYu\*.*" 0 skipWorkspaceDelete
            MessageBox MB_YESNO "是否需要彻底删除默认工作空间（$PROFILE\QingYu\）？$\n$\n\
                Do you want to completely delete the default workspace ($PROFILE\QingYu\)?$\n" \
                /SD IDNO IDYES AcceptedRMWorkspace IDNO SkippedRMWrokspace
                AcceptedRMWorkspace:
                    RMDir /r "$PROFILE\QingYu\"
                SkippedRMWrokspace:
        skipWorkspaceDelete:
    ${EndIf}
